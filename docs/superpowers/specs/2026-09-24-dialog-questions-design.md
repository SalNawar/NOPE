...
| R14 | **The id grammar (Q16)** has one home per grammar (§2.13): the generator's helpers for the ids only it writes, and Domain `InterviewScript` for the ids the runtime composes too, which the generator calls. Every id, generated or authored, is unique across the whole world source, and an authored line id starts with its dialog's id and a dot. At runtime the transcript names each row `Line_{id}`, which the verification uses to find rows. | Ids get a real reader in piece 3 without dead code. Piece 6's string tables will key on these ids, so two strings must never share one. |
...
| R25 | **Menu capacity.** The intercom's `Actions` list fits 8 buttons (about 432 px after padding; 44 px buttons with 6 px spacing) on every screen: both canvas scalers use `ScreenMatchMode.Expand`, so the canvas is never shorter than the 1080 px reference (§2.12). A content knob, `interview.menuCapacity` (8), is checked three ways: the generator and the validator reject content whose ask menu (back + questions + small talk) or hub (the most document templates of any blueprint the day plans or legendaries use + ask + every dialog) or any authored node exceeds it (`DialogChecks.MenuProblems` and `Problems`), and the builder logs an error when the intercom it lays out fits fewer buttons than the knob. "< Back" is the ask menu's **first** choice and document requests are the hub's first choices, so an overflow clipped by the new `RectMask2D` can hide a question or a dialog but never the way back or a document. | With Interview Protocols the ask menu holds exactly 8 entries. One more question in `world_source.json` would otherwise hide "< Back" without any error, trap the player in the ask menu and make a Papers-tell liar unwinnable (documents open only through the intercom). The hub count is conservative (every dialog counted as offered at once); piece 4 revisits it when arcs outgrow it. |
...
- **Determinism:** same run + same day (and the same interview wiring, R12, and the same liar chance) = same travellers, same liars, same tells, same small talk; with the same askable questions, also the same answers. The liar chance is piece 2's: the blueprint's contradiction chance plus the slot-machine modifier and active forgery-risk (`ForgeryChanceBonus`) effects (`CaseFactory.LiarChance`), so a slot outcome or a forgery-risk trigger can still change who lies.
- **Stream contract:**
  ...
  - unlocking a question never changes the lie draws: only day-gated questions add Answer options (R23), so which categories may carry a spoken tell depends only on the day. Buying Interview Protocols adds the birth-date answer to every traveller and nothing else. (Who lies still depends on the liar chance, which keeps piece 2's slot-machine and forgery-risk inputs; see Determinism above.)
...
| Lies (`_lieRng`) | `ForLies` | roll; home; one per tell over (category, channel) options; BirthDate year | the option pool grows when Answer is allowed and day-gated questions are askable; identical to piece 2 otherwise; the option pool never depends on purchases or flags (R23), while the roll's liar chance keeps piece 2's slot-machine and forgery-risk inputs |
...
- **Intercom capacity (R25):** both canvas scalers (the desktop canvas and the office overlay) are set up by one helper, `ConfigureScaler`: scale with the screen from the 1920×1080 reference, `ScreenMatchMode.Expand`, so a canvas is never smaller than the reference in either dimension. (With the default match-width setting, a 21:9 screen gave a 1920×810 canvas whose `Actions` rect held 6 buttons, and the mask hid and blocked the rest.) The builder computes how many action buttons its `Actions` rect fits at the 1080 px reference height, the least height the canvas has, from the numbers it lays out (panel anchors 0.36–0.85, `Actions` anchors 0.02–0.86, padding 6 + 6, spacing 6, button height 44): `floor((height − 12 + 6) / (44 + 6))` = 8. The anchors, spacing and button height become named local constants shared by the layout calls and this computation. After the library lookup (223), when `library != null` and `library.Interview.menuCapacity` exceeds the fit, it logs `"[TimeDesk] The intercom fits {fit} choices, but the content library's interview menu capacity is {n}; lower interview.menuCapacity in world_source.json or enlarge the intercom."`.
...
**Id grammar (Q16).** Each grammar has one home, and every site that writes or checks an id calls it. The generator's private helpers `InterviewLineId`, `QuestionLineId`, `OverrideLineId` (with the parts `PromptPart`/`AnswerPart`), `SmallTalkId` and `PlaceId` hold the ids only Generate World writes; Domain `InterviewScript.ChoiceLineId`, `IntroLineId` and `ClaimLineId` hold the ids the runtime composes (`InterviewScript.BuildNode` and `Opening`), and the generator reserves and checks exactly those (tested: `InterviewScriptTests`):
...
| `Assets/Scripts/Domain/InterviewScript.cs` (+meta) | Domain | new: `InterviewCase`, `InterviewScript` (with the runtime line ids `IntroLineId`, `ClaimLineId`, `ChoiceLineId`), `DialogChecks` (`Problems`, `MenuProblems`) |
...
- **§1.4 lines 108-110:** days 2–3 lie draws differ from piece 2's (their day-gated questions add Answer options), and answers also depend on the day's askable questions; the tell options still never depend on purchases or flags (§1.8, R23).
...
### Implemented differently (plan review, 2026-09-24)

The plan (`docs/superpowers/plans/2026-09-24-dialog-questions.md`, "Notes for the reviewer") settled some points differently from the design above, and the code follows the plan. The approved lines above stay as the record; these entries supersede them. Line numbers are this file's.

- **Plan note 1: the transcript window is rebuilt on every Build Office UI run.** Superseded: §2.12 steps 4–5 (lines 616–617: "Get or add a `TranscriptWindowController`" and "On every build, re-apply the row template's layout"). The builder destroys `IconClueLogWindow` and `TranscriptWindow`, builds the window fresh, adds the controller, sets its references and `entriesPerPage = 8`, and lays out the fresh row template (`ApplyTranscriptRowLayout`). A window that was found and updated would sit at a different place in the window layer after the first and the second build (the Scanner, Records and document-template windows are recreated and appended on every run), so the §6 step 5 dumps would differ. Rebuilt, every build gives the same order. The list of objects recreated on every run in §6 step 5 (line 1140) gains `TranscriptWindow`.
- **Plan note 9: one interview projection, the public `TimelineService.BuildInterviewDay(ContentLibrarySO, WorldState, ShiftLedger)`.** It wraps every library question and dialog as a `Gated<T>` with `ToGates`, takes one `Snapshot` of the world for all their conditions, and builds the `InterviewDay`. `GameManager.BuildInterviewDay` only calls it and logs `ContentProblems`, and the Unity verification calls the same method. So `ToGate`, `ToGates` and `Snapshot` are private. Superseded:
  - §2.8 lines 466, 472 and 473 (`public static`, and the caller "`GameManager.BuildInterviewDay`");
  - §2.10 steps 1–3 (lines 528–530: the snapshot and the wrapping in `GameManager`);
  - §2.2 line 242 ("`GameManager` wraps each `QuestionSO`/`DialogSO`");
  - R7's rationale (line 60, "`GameManager` only projects conditions and copies the snapshot"): `TimelineService.BuildInterviewDay` does both, and `GameManager` calls it;
  - §2.18 lines 882–883: the projection and the snapshot are `TimelineService.BuildInterviewDay`'s glue, and `GameManager.BuildInterviewDay` keeps only the error log;
  - the §2.17 `TimelineService.cs` row (line 842), which gains `BuildInterviewDay`;
  - V3 (line 1246): `ToGates`' callers are `AllConditionsPass` and `BuildInterviewDay`, both inside `TimelineService`.
- **Plan note 10: missing wording warns once a day.** Superseded: §2.9 lines 494–495 ("A blank result logs one warning" for the opener and the claim). `CaseFactory.GenerateDayCases` checks the library's opener, legendary opener and claim once a day and logs "Day {n}: …" warnings that name Generate World; `Interview.Opener` and `Claim` log nothing. A warning per traveller would repeat up to 12 times a day for one content gap.
- **Plan notes 4 and 11: one home for each rule the generator and the validator share.** These public members are missing from §2.13 and from the §2.17 table (lines 824–872):
  - `Interview.HoldsToken(template, token)` (Domain, tested in `InterviewTests`): "the template holds `{token}`", for both the generator and the validator. The "template holds its token" checks that §2.4 line 331 lists as `Placeholder` callers call `HoldsToken`, which calls `Placeholder`. The §2.17 `Interview.cs` row (line 830) gains it;
  - `ContentLibraryValidator.MaxDocuments(IEnumerable<CaseBlueprintSO>)`: the most documents one traveller carries. The generator's menu check (line 685) and the validator's (line 708) each pass their own blueprints;
  - `ContentLibraryValidator.DialogEffectOpError(dialogId, choiceId, effectName, op)`: the one text of the R24 error ("same message as the generator's", line 709), which the generator also calls;
  - `DayPlanSO.ForcedBlueprints`: the forced cases' blueprints, which the validator's menu check counts. The §2.17 `DayPlanSO.cs` row (line 844) gains it;
  - the private `WorldContentGenerator.BirthYears`, one birth-year range for `MakePlace` and the line-length check.
- **Plan notes 2 and 3: check messages.** The example message at line 653 names the line's node: "… used by dialog 'dlg_rumour' node 'start' and by choice 'more' …", not "line 'start'". §6 step 1's seventh-question negative check (line 1118) gives two errors: the capacity error and "asks about Technology again", since six questions already cover every provable category.

### Code review (2026-09-24)

Nine findings, one of them reported twice. Each was re-checked against the code before it was fixed. All are applied; where a finding's detail was wrong, the Verdict column says so.

| # | Finding | Verdict | Where |
|---|---|---|---|
| C1 | `InterviewLines.maxLineChars` had no reader: only the validator's own `>= 1` check read the asset copy, while Generate World's line-length check reads the source's `interview.maxLineChars` | applied, option (a) | The field is gone from `InterviewLines`, `BuildLines` and the validator. The limit lives in `world_source.json` only. §2.4 (line 320), §2.13 (lines 634 and 704), §2.15 (line 800) and §6 step 1 were edited in place; FEATURES and the plan's Task 17 check were updated too (`3a18c7d`). The regenerated library drops the key (`130593b`) |
| C2 | The line-id grammar was typed as literals at many sites: the choice-label id at runtime and three times in the generator, the question, override and place ids at every check and build site | applied | Domain `InterviewScript.IntroLineId`, `ClaimLineId` and `ChoiceLineId` (tested in `InterviewScriptTests`); the generator's `QuestionLineId`, `OverrideLineId` and `PlaceId`. R14, §2.13 and §2.17 edited (`4ac20e3`). Unity: the generated ids are unchanged, and the id-collision checks catch a clash with a choice, a question or a runtime line |
| C3 | FEATURES :11 and §1.8 said the lie draws depend only on the run and the day. The liar chance also reads the slot-machine modifier and forgery-risk effects (reported twice) | applied | FEATURES :11, R23, §1.8, the §2.9 streams table, §3.2 and §3.3 now claim what `InterviewDayTests` covers: only day-gated questions may carry a spoken tell, so an unlock by purchase or flag never changes the tell options (`7895004`). The shipped forgery-risk trigger is gated on an attribute threshold, not a flag; the claim was wrong either way |
| C4 | §3.2 missed earlier spec lines that piece 3 changes | applied | §3.2: office-scene spec lines 112, 114 and 120–122; identity-lies lines 332 and 598; world-model line 74 (`411f1c5`) |
| C5 | The spec still described what the plan changed, and nothing was appended | applied | "Implemented differently (plan review)" above |
| C6 | The answers bullet cited `InterviewTests` for `CaseFactory` glue that the EditMode suite cannot see | applied | FEATURES and §3.3: `InterviewTests` for the answer rule and the small-talk order, `InterviewScriptTests` for "never evidence", and the Unity checks (§6 steps 3.5 and 3.10) for which values and lists `CaseFactory` passes (`5323e39`) |
| C7 | The intercom's capacity held only up to about 1.95:1. The canvases matched width, so a 21:9 screen got a 1920×810 canvas that fits 6 buttons, and the `RectMask2D` hid and blocked the rest | applied (`ScreenMatchMode.Expand`) | The builder's `ConfigureScaler` sets both scalers to Expand, so the canvas is never smaller than 1920×1080. R25, §2.12 and FEATURES updated (`b4a1c5e`). The scene was rebuilt twice with equal semantic dumps, equal to the previous build apart from the scalers (`05eeea3`). The finding overstated its effect: with Interview Protocols the fully hidden entry is Small talk (Date of birth is about 60% visible). One-shot questions leave the menu, so the entry moves into view after two questions are asked, and at 2:1 the last entry is only slightly clipped. The fix removes the case anyway |
| C8 | Two day-3 fact values made broken answers: "Our capital is No capital (tribal things)." and "We trade with Barter: iron ingots & cloth." | applied, plus a third | "Semnones' sacred grove", "Iron ingots & cloth (barter)" and "Cairo (United Arab Republic)" (the last ended its sentence in ".."). The §2.14 note records the authoring rule. Regenerated; Validate Content Library reports no issues (`130593b`) |

Verified offline (compile check 0 errors, 337/337 Domain tests) and in the worktree's Unity 6000.4.11f1 through a temporary `-executeMethod` script (not committed). Generate World ran twice, and the second run changed no file. The library no longer serializes `maxLineChars`. The line ids of questions, overrides, dialogs and small talk are unchanged. The three reworded values reach the places and the day-3 fact table. Validate Content Library reports no issues. The generator's negative checks pass: a line id that equals a choice-label, question or runtime id, a 101-character line, and a 104-character override answer. Build Office UI ran twice with no error, and both scalers are Expand.

## 8. Verification (2026-09-24)

Run in the branch's own Unity 6000.4.11f1 editor through temporary `-executeMethod` scripts (not committed), at `4cf9461` (after the code review's fixes):
...
- Content (step 1): Generate World ran twice and the second run changed no file; `Assets/Data/World/Interview` holds 6 questions, 2 dialogs and 3 unlock triggers; the library holds 5 books, 6 questions, 2 dialogs, the 3 authored triggers then the 3 unlock triggers, 4 upgrades and the rumour effect, with menu capacity 8; lines are limited to 100 characters (Task 17 read the limit from the library; since C1 it lives only in `world_source.json`, and the review run confirmed the library no longer serializes it); both dialogs are one-shot; every question's and unlock trigger's conditions are as §2.14 says; the day plans hold their channels. Validate Content Library: no issues. The generator's checks on five edited copies of the source each gave exactly the expected error(s) (a missing `fromDay`; a seventh question, whose repeated category adds the one-per-category error; a line id equal to a choice-label id; a 101-character line; a dialog effect with a PayRateBonus op). The review run added a line id equal to a question line id, one equal to a reserved runtime id, and a 104-character override answer, each giving exactly the expected errors.
- Gates and triggers (step 2): the authored triggers' 60 results equal the baseline. Unlock notices fire the night before the first askable day (capital on the night of day 1, ruler on the night of day 2, birth date the night Interview Protocols is owned, never without it), once each. The day-start interview asks [Currency, Language, Technology] on day 1, [+ Geography] on day 2 and [+ Politics] on day 3, each + BirthDate with Interview Protocols, and lets spoken tells come only from the day-gated questions, with or without the upgrade. No dialog content problem on any day.
- World check (step 3; seeds 12345 and 999 × days 1–3 with and without Interview Protocols, plus 200 seeds × days 1–3 × both): 0 problems. Deterministic; identities equal the baseline on every day, and day 1's lies and papers equal it in full; the upgrade changes no lie and only adds the birth-date answer. Every spoken tell proves against the claim's row and names the true home on its row, and nothing else; honest answers prove nothing; no Politics tell between Egypt and Greece (Early modern). Sweep: spoken-tell share day 1 0.000 (0/810), day 2 0.490 (409/835), day 3 0.520 (467/898), the same counts with the upgrade; options day1 BirthDate/Papers, day1 Currency/Papers, day1 Language/Papers, day1 Technology/Papers, day2 BirthDate/Papers, day2 Currency/Answer, day2 Currency/Papers, day2 Geography/Answer, day2 Language/Answer, day2 Language/Papers, day2 Technology/Answer, day2 Technology/Papers, day3 BirthDate/Papers, day3 Currency/Answer, day3 Currency/Papers, day3 Geography/Answer, day3 Language/Answer, day3 Language/Papers, day3 Politics/Answer, day3 Technology/Answer, day3 Technology/Papers; no BirthDate Answer tell; no NoPossibleLie warning. Review C1/C8 (a check added to the plan's script): every line of every checked interview (opener, claim, small talk, each question's prompt and its rendered answer; 148,672 lines) fits the 100 characters `world_source.json` sets, the longest being a 67-character claim, and none holds "..".
- EditMode suite (step 4): 466 passed. The one failure is the known third-party `UnitySkills.Tests.Core.PerceptionSkillsTests.SceneSummarize_CountsObjectsCorrectly`. Offline Domain run: 337/337 (the plan's 336 plus the review's `InterviewScriptTests` case for the line ids).
- Builder (step 5): Build Office UI ran twice on OfficeScene with no error; the two semantic scene dumps (900 lines) are equal. `TranscriptWindow` is wired (8 rows per page, built closed), `IconClueLog` opens it, `IconClueLogWindow` is gone, the intercom's `Actions` is masked, the icons require `archive_access` / nothing / `adv_scanner`, and the Dialect window reads "Dialect" / "Notes on accents and phrasing. (placeholder)". A third, unsaved build with an in-memory menu capacity of 9 and Archive Access under another id reported exactly the builder's two errors (the intercom fits 8 choices; Lexicon's upgrade is unknown) and changed no file. The rebuilt scene is committed (`63ab97c`). After the review's C7 fix, two more builds gave equal dumps (902 lines) with both canvas scalers on Expand, which gives a canvas of at least 1920×1080 on 2560×1080, 3440×1440, 1920×960, 1920×1080 and 1280×1024 screens (`05eeea3`). This verification changed neither the builder nor the source, so the scene was not rebuilt.
- Play-through (step 6; run seed 77, a new run started on day 2; slot 1 Hegeso, honest, claiming Periclean Athens; slot 2 Hui, claiming Eastern Han Luoyang with a spoken capital as the only tell; slot 3 Phanostrate, claiming Periclean Athens with a first-page Declared Device tell): slot 1's hub listed the two requests, "Ask about home >" and "Any news from home? >", and its ask menu "< Back", Currency, Language, Device, Capital, Small talk; "Request Travel Passport" opened the passport and added the prompt and reply while the transcript stayed closed. Hegeso's four answers each gave MATCH against their claim rows and logged nothing; accepted, correct. Hui said "Our capital is Cairo (Mamluk Citadel)."; the answer row against the Gazetteer's claim row logged `CAPITAL INCORRECT — traveller said: "Cairo (Mamluk Citadel)"  /  expected: "Luoyang"`, and the true home's row against the same answer showed `ALREADY DOCUMENTED — CAPITAL is in the Deviation Report`; denied, correct, no citation. Phanostrate answered Device with the cover "Klepsydra (water clock)" (MATCH, nothing logged), while the paper's "Essedum (war chariot)" against the book logged `DEVICE INCORRECT`; denied, correct, no citation. No transcript row overflowed, including the 70-, 92- and 100-character worst cases (15 pt, two lines). Escape and back kept the transcript. The rumour dialog was completed and left the next traveller's hub. After forced closing the save held the done flag, `rumour_calculators` and the rumour effect starting on day 3; day 3's paper carried the rumour tip and the ruler notice, offered the follow-up dialog (not the rumour) and asked about the ruler. No warnings or errors.
- Review fixes in play (checks added to the plan's script): the first transcript lines and rows carry the runtime ids `case.intro`, `case.claim`, `interview.requestPrompt` and `interview.requestReply`, and the rumour's lines `dlg_rumour.start.1`, `dlg_rumour.more`, `dlg_rumour.detail.1` and `dlg_rumour.noted` (C2). The Game view was 1920×1080 and the desktop canvas 1920×1080 on Expand; all 4 hub choices lay wholly inside the masked `Actions` area, and so did all 8 choices of day 3's ask menu (< Back, Currency, Language, Device, Capital, Ruler, Date of birth, Small talk) after Interview Protocols was bought before the night of day 2, a step added so the menu reaches the capacity (C7). That morning's paper also carried the birth-date notice.
- Seen, not a defect: day 3's paper also carried the capital notice, a night late. The run started on day 2 through `RunConfig.startingDay`, so the night of day 1 never ran and `unlock_q_capital` fired on the night of day 2. This is the late announcement §7 describes for older saves; the capital question itself was askable on day 2 as R5 says.
- Hygiene (step 7): the temporary scripts and their metas were deleted; the Unity-touched `*.csproj`, `ProjectSettings/*`, the TMP fallback atlas and `RunConfig.asset` were reverted, and the player's save was restored. The verification changed no committed file, and no product defect was found.
