...
## Implementation amendments (piece 4 build, 2026-09-25), binding over the rest of this draft

These rows apply `piece4_decisions.md` Amendments A1 (the cast), A3 (the live booth is the builder-built 2D booth with the painted art) and A4 (piece 7 is on main: no Visitor window, garments through the traveller wheel), and record where the code departs from the draft. Where a row and the draft disagree, the row wins. §2.14's cast, pools and remarks are edited in place (A1); the other sections keep their text as the reviewed record.

**A4: no Visitor window; "Look >" on the traveller wheel.**

| Id | Decision |
|---|---|
| K1 | The Visitor window, its desktop icon, its auto-open (R12), `VisitorReachable` (R14) and the Visitor wiring warning are dropped. `TravellerPortraitView` is only the passport photo on the scanned page (no buttons, no highlights). `Looks.IsGarmentLayer` is dropped too: the look menu lists `TravellerLook.Garments`, so nothing reads layers as garments. |
| K2 | The hub gains **"Look >"** (`InterviewLines.lookLabel`, authored as `interview.lookLabel`), after "Ask about home >" and before the dialogs, whenever the traveller has a visible garment. It opens node `look`: "< Back" in the wheel's centre, then one choice per visible garment in `LookSlot` order, labelled with the item's own label (a premade: `wholeFigureLabel`). A look choice has `DialogAction.InspectGarment` and `DialogChoice.GarmentIndex`, is never one-shot and adds no transcript line. `InterviewScript.Build` builds it from the new `InterviewCase.garments` (tested). |
| K3 | Choosing a garment puts it into the PC compare bar through the same path as every row: label `Traveller · {Looks.SlotLabel(slot)}`, shown value the item label, evidence `CompareEvidence.ForAppearance(Culture, garment.Value, garment.IsTell)` (R2's `MatchValue` rule unchanged). The wheel then closes (like a hand-over), so the player turns to the PC and clicks a Costume Guide row; reopening the wheel shows the look menu again. |
| K4 | Menu capacity: `DialogChecks.MenuProblems(questions, smallTalk, maxRequestedDocuments, dialogs, premadeDialogs, maxChoices)` counts the hub as requests + ask + look + the dialogs no premade names + 1 when any dialog names a premade (R18), and checks the look menu (1 + one per `LookSlot` = 6). |
| K5 | Dress tells are reachable where the look menu and the compare bar are: `InvestigationUIController.AppearanceReachable` is true in the text fallback and, in the rich desk, when the wheel's ring and the compare controller are wired; `GameManager` passes it as `GenerateDayCases(..., appearanceReachable)`. A rich scene without it logs one warning and generates no dress tell (R14's pattern). |
| K6 | Premade expressions: the last traveller line with an expression raises `InvestigationUIController.TravellerExpressionChanged`, and `GameManager` forwards it to `TravellerView.SetExpression` (the booth is the only full view; the photo keeps the neutral image). |

**A4: the passport photo in two places.**

| Id | Decision |
|---|---|
| K7 | The photo is made of cropped sprites, not an anchored full stack (supersedes R13's `RectMask2D` and `LookCanvas.PhotoAnchorMin`/`Max`): `CharacterArt.GetPhoto(key)` makes a sprite of each layer's photo rect (canvas (362, 215)–(662, 590)), cached and released with the layer. `LookCanvas.PhotoRect` (normalised, bottom-left origin) is tested. |
| K8 | Scanned page: the builder's existing top-right `PhotoBox` stays where it is (the scanned page's layout does not move) and holds a 4:5 `Portrait` (`AspectRatioFitter`) with one `Image` per `LookLayer` (`TravellerPortraitView`); the "PHOTO" label goes. `DocumentWindowController.photoInset` (120 px) pads the rows on the right, on page 0 of a photo document only. |
| K9 | Physical paper: `DeskDocument.photoSlot` (the grey frame) shows when the paper's document has `showsPhoto`, with a `Photo` child, a `LookSpriteStack` of crop sprites inside the paper's sorting group. `CaseDocument.showsPhoto` carries the template flag; `DeskController.BeginCase(docs, look, art)` receives the look. |

**A3: the booth figure in the painted booth.**

| Id | Decision |
|---|---|
| K10 | `OfficeRoot/Traveller` (`TravellerView`, scale 1) keeps `Anchor` and `TravellerHitZone`; the layers live on a new child `Figure` (`SortingGroup` order −20, `LookSpriteStack`, one `SpriteRenderer` child per `LookLayer`), so the hit zone stays outside the sorting group (piece-7 K20). The figure's feet stand at world (0, −3.9, 2) and its canvas is 8 units tall: head top near 2.5 and hips at the desk art's far edge (−0.84), the purple placeholder's place and size (judged by screenshot, §6). These are builder constants and supersede R11. |
| K11 | Existing-wins: the builder creates `Figure` with that placement only when it is missing; an existing figure keeps its transform. The anchor (the figure's shoulders) and the hit zone (the figure above the desk's far edge, ±232 canvas px wide, up to 120 px into the headroom) are refitted from the figure on every build. The placeholder `SpriteRenderer` on `Traveller` is removed; `traveller.png` stays (R26). |
| K12 | `TravellerView.figure` becomes a `LookSpriteStack`; `Show(look, art)`, `SetExpression`, `Clear` and `Anchor` as in §2.9. `GameManager.SetTravellerAtDesk(bool)` stays the one presence transition; the look reaches the booth through `ShowActiveCase`. |
| K13 | `LookSpriteStack` (new, `Assets/Scripts/Characters`) is the one sprite-layer renderer: the booth figure (full sprites) and the paper photo (crop sprites, its `photo` flag) share it. |

**Other departures.**

| Id | Decision |
|---|---|
| K14 | Wardrobe import (R23): the reviewed art-brief draft `docs/superpowers/drafts/art/tools/wardrobe_v2.json` already holds every place's `worldSourceWardrobe` block (the 68 fixes applied), its Culture value and the confusable lists (`confusable`, `confusableHidden`), so a one-off scratchpad script copies those into `world_source.json` (no second import from `costume_research.json`). The key grammar of `coverage.json` equals `LookKeys`; where they ever disagree, the code wins. |
| K15 | Placeholder skin uses the art brief's five swatches (`PlaceholderPalette.Skin`), hair its five colours (`PlaceholderPalette.Hair`). |
| K16 | `InterviewLines.maxLineChars` does not exist in the code: the line cap is the source-only `world_source.json interview.maxLineChars`, which the generator reads for the premade intro and name checks. |
| K17 | §2.12's `BuildPortraitStack(clickable: true)`, the Visitor window block, its icon and the `visitorView`/`visitorChrome` wiring are dropped (K1). |

**A1: the cast** (§2.14, edited). Ten premades, five women and five men: Senenmut (story, day 1 slot 3), "Socrates" (the story impostor, day 2 slot 6, pooled on day 3), Aspasia, Ban Zhao, Arib al-Ma'muniyya and Muhammad al-Khwarizmi (pools of days 2 and 3), Johannes Gutenberg, Leonardo da Vinci, Aemilia Lanyer and Cecilia Gallerani (pool of day 3). Cai Lun and Zeami Motokiyo leave the cast (the art brief keeps them as alternates). C14's sourcing is relaxed to real, long-dead, non-ruler people who lived in the place and era (born inside its birth years), not only people its moment names.
...
- **Who**: real, long-dead people who lived in their claimed place and era, drawn whole, five women and five men (A1): Senenmut, Socrates, Aspasia, Ban Zhao, Arib al-Ma'muniyya, Muhammad al-Khwarizmi, Johannes Gutenberg, Leonardo da Vinci, Aemilia Lanyer, Cecilia Gallerani (§2.14; Saleh reviews the list).
- **When**:
...
**Premades** (A1: five women, five men; archetype by `ArchetypeSO.id`):

| id | name | gender | claim | true place | born | role | schedule | intro / record note |
|---|---|---|---|---|---|---|---|---|
| senenmut | Senenmut | m | egypt_ancient | — | 14 Mar 1505 BCE | artist | forced day 1 slot 3; dialog `dlg_senenmut` | — / "Steward of the Pharaoh's household and architect of her temple." |
| socrates | Socrates | m | greece_ancient | italy_ancient (an impostor) | 6 Jun 470 BCE | wanderer | forced day 2 slot 6; pooled day 3 | "Priority arrival: Socrates of Athens. He asks more questions than you do." / "Philosopher. Known to question officials at length." |
| aspasia | Aspasia | f | greece_ancient | — | 12 Sep 470 BCE | diplomat | pooled days 2–3 | — / "Teacher of rhetoric. Keeps the company of Athens' first citizen." |
| banzhao | Ban Zhao | f | china_ancient | — | 3 Feb 45 | scientist | pooled days 2–3 | — / "Court historian and tutor to the empress. Finishing her brother's history." |
| arib | Arib al-Ma'muniyya | f | iraq_medieval | — | 20 May 797 | artist | pooled days 2–3 | — / "Singer and poet of the caliph's court. Travels with her own musicians." |
| khwarizmi | Muhammad al-Khwarizmi | m | iraq_medieval | — | 3 Apr 780 | scientist | pooled days 2–3 | — / "Scholar of the House of Wisdom. Writes on calculation." |
| gutenberg | Johannes Gutenberg | m | germany_medieval | — | 24 Jun 1400 | scientist | pooled day 3 | — / "Goldsmith of Mainz. Business in Strasbourg undisclosed." |
| leonardo | Leonardo da Vinci | m | italy_earlymodern | — | 15 Apr 1452 | artist | pooled day 3 | — / "Painter and engineer in the service of the Duke of Milan." |
| lanyer | Aemilia Lanyer | f | britain_earlymodern | — | 27 Jan 1569 | artist | pooled day 3 | — / "Poet of London, raised in a countess's household." |
| gallerani | Cecilia Gallerani | f | italy_earlymodern | — | 3 Mar 1473 | diplomat | pooled day 3 | — / "Lady of the Milanese court. Sat for a portrait with an ermine." |

Every entry authors `repeatable: false` and `impacts: []` explicitly (R29); the intro and notes are ASCII, Socrates' intro is 73 characters (≤ `interview.maxLineChars` 100), the longest name 21 ("Muhammad al-Khwarizmi", so the legendary opener fills to 40) and the longest note under `Premades.MaxNoteLength` (120). Every birth year lies in its claimed place's birth years (year − 70 .. year − 18; for example −1505 in −1540..−1488, 45 in 35..87, 1569 in 1530..1582). The dates are the art brief's (section 11): traditional where known, otherwise invented inside the range. No name is in any place's name list (Generate World checks). The people are real, long dead and not rulers (A1): the moments name Senenmut, Socrates, al-Khwarizmi, Gutenberg and Leonardo; Aspasia, Ban Zhao, Arib, Lanyer and Gallerani lived in their place and era. Rulers (Hatshepsut, Tokugawa Ieyasu, Nasser) are excluded by C14. Day 3's rules (no Medieval China, no Early modern Japan) forbid none of the pooled claims, and day 2's (no Ancient Egypt) none of day 2's. Industrial and Modern premades belong to piece 5's days.
...
**Days:** day 1 forced `[{slot 3, premade senenmut}]`, pool `[]`; day 2 forced `[{slot 6, premade socrates}]`, pool `[aspasia, banzhao, arib, khwarizmi]`; day 3 forced `[]`, pool `[socrates, aspasia, banzhao, arib, khwarizmi, gutenberg, leonardo, lanyer, gallerani]`; `premadeChance` 0.05 authored on all three (today's value). No forced blueprint is authored (none is today). Day 1's slot 3 lies in the first half of its queue (`ViolatorSlots.Window(8)` = 4), which draws no warning because day 1 has no rules.
...
- **New "Characters" group after "World"**:
  ...
  - "Premade characters: ten named people drawn whole with four expressions; story slots (day 1 slot 3, day 2 slot 6) and a 5% chance per slot from the day's pool; once per run when presented; never on a rule violator's slot; honest unless authored as liars (papers or answers only); their own opener and record note; a premade conversation changes their expression (scheduling tested: `PremadesTests`, `ViolatorSlotsTests`; the opener tested: `InterviewTests`)".
...
1. **Content**:
   ...
   - `Assets/Data/World/Premades` holds 10 premades (`oncePerRun` true on all); the library lists them, 6 books and `lookRules`; the day plans hold the §2.14 pools, story slots (premade set, `caseBlueprint` null) and chance, and day 3's channels;
...
## Verification record (piece 4 build, 2026-09-25)

**Commits** (branch `feat/characters`, on `8f98bbc`): `4f530af` spec alignment (A1/A3/A4); `e7c4b77` Domain look model, key grammar, premade rules, seeds; `ca3f7dc` Domain Appearance channel, garment evidence, look menu; `429694b` Visuals canvas and placeholders; `a2d0420` content, generator, validator and case generation; `f1dfb22` runtime (art, views, look menu, photos); `d378626` builder; `97c255c`, `0a85bd8` fixes found by the scene checks; `388e4b0` review fixes; `67306db` regenerated content; this record.

**Offline** (after every commit): `compile_check.py` 0 errors in every project; the reflection runner 576 passed, 0 failed (434 before the piece).

**Unity 6000.4.11f1, scene-independent** (temporary `_TimeDeskP4Automation`, never committed; report `p4_automation_report.txt` in the session scratchpad):
- Generate World twice: the second run changes no file under `Assets/Data`; no error. Validate Content Library: no issues; its art report logs 0/848 keys with final art (placeholders drawn for all).
- Content: 40 places with year, wardrobes, weights and a Culture fact equal to `Looks.CultureValue` (40 distinct values, the longest 28 characters, `Looks.LabelProblems` empty); 10 premades (5 women, 5 men, all once per run) in `Assets/Data/World/Premades` and the library; six books with the Costume Guide; look rules (3 bands, grey 60, "Period dress", 130 confusable pairs); day 1 Senenmut in slot 3, day 2 Socrates in slot 6 and a pool of four, day 3 a pool of nine and the dress channel; chance 0.05; the passport shows a photo, the permit none.
- World check (seeds 12345 and 999, days 1-3, fresh state): two dumps identical; with Senenmut, Socrates and al-Khwarizmi met, two dumps identical and the forced slots hold ordinary travellers; a different seed differs; Socrates is a liar from Republican Rome through papers or answers only, drawn whole, with his note and opener; day 2 at chance 1 over 50 seeds: 250 premade slots, every violator slot kept its violator, no premade twice a day.
- Stream contract against the piece-3/7 baseline (`p7_cases_now_run2.txt`, compared by `p4_contract.py`): 0 violations. Every non-premade slot keeps its claim, birth date, role, rule verdict and violator status; days 1-2 keep lies, tells, answers and papers (up to the name); one later same-place traveller is renamed after a premade slot (seed 999 day 1 slot 5, Hipponikos to Myrrhine); on day 3 eleven liars change tells (two become dress tells) and every traveller honest in both runs keeps its answers.
- 200-seed sweep, days 1-3: every key is enumerated by `LookKeys`; skin, hair and face follow the claim's weights and the cover age (grey from 60); honest and non-dress travellers wear only the claim; premades are one whole picture, never on a violator's slot or twice a day; no dress tell on days 1-2 or with `appearanceReachable` false. Day 3: 80 dress-tell liars of 864 liars (9.3%), each with exactly one tell garment, the true home's signature for the gender, `CanLeak` true, proving ClaimMismatch against the claim's Costume Guide row and ForeignOrigin naming the home against the home's row and nothing against any other row; honest garments prove nothing; all 24 day-3 places with a leakable signature appear as dress homes; 152 random premades over 600 days.
- EditMode suite through `TestRunnerApi`: 705 passed, 1 failed: the known third-party `UnitySkills.Tests.Core.PerceptionSkillsTests.SceneSummarize_CountsObjectsCorrectly`.

**Scene checks on the outdated 2D booth, run before Saleh's instruction to stop them** (the coordinator, 2026-09-25: the builder-built 2D booth is outdated; the game moves into Codex's hybrid 3D office right after this piece; no 2D scene testing, rebuilds or commits): Build Office UI twice gave identical semantic scene dumps (1712 lines) and the reloaded saved scene the same; the figure (sorting group -20, nine layer renderers, feet at (0, -3.9, 2), canvas 8 units), its anchor and refitted hit zone, the paper's and the scanned page's photo stacks and `GameManager.travellerView` were wired. A scripted play-through of days 1-3 (seed 12345) passed every check across its runs: the layered figure in the booth, the passport photo on the paper and on the scanned page (after `97c255c`), the permit without one, "Look >" listing garments by name, a look closing the wheel and filling the compare bar, an honest garment MATCHing its own Costume Guide row, Senenmut's dialog and expressions (after `0a85bd8` the mark shows), his record note, bonus and met flag, Socrates' spoken Device tell logged and denied, the met flags in the save, a day-3 hairstyle leaked under the claimed headwear proven against the true home's row ("DRESS INCORRECT — traveller wears "mizura / magatama beads", which belongs to Kofun Yamato (Ancient)"), ALREADY DOCUMENTED against the claim's row, correct denials without citations, no warning or error. The last re-run stopped on a harness race after the proof (a click issued before the view left focus); it was fixed in the harness but, per the instruction, not re-run. Screenshots: `p4_booth_traveller.png`, `p4_wheel_look.png`, `p4_passport_scan.png`, `p4_premade.png`, `p4_premade_dialog.png`, `p4_dress_tell_booth.png`, `p4_dress_proof.png` (scratchpad).

**Not done, by instruction:** `Assets/Scenes/OfficeScene.unity` is **not** rebuilt or committed. Until the scene is built with this builder (or the move to the hybrid office wires it), `TravellerView.figure`, the photo stacks and the document window's portrait are unwired in the committed scene: the booth keeps the old purple placeholder sprite (now never hidden, since `TravellerView` no longer toggles renderers), the papers and scanned pages show no photo, and the rest of the piece works. The booth placement constants (K10), the hit-zone refit and the day-1 wheel note (which overlaps a tall hat in the 2D booth) are to be redone in the hybrid office; all scene verification (step 4 and step 5 of §6) moves there.

**Departures found while building** (beyond the implementation amendments): `Looks.IsGarmentLayer` was dropped (the look menu lists `TravellerLook.Garments`, so nothing reads layers as garments; K1's sentence about it is void); `LookKey` has no `Face` or `PremadeId` (nothing reads them); the safe area lives in the art contract, not in `LookCanvas`; `InvestigationUIController.AppearanceReachable` (wheel ring and compare bar) replaces `VisitorReachable`; the scanned page's photo box stays top right with a right-hand row inset (K8); premade placeholders keep the head unstriped so the expression mark shows.

## Applied at the piece 4 + 5 merge (2026-09-25)

Piece 5's §8 was applied after both pieces met (history spec, "Applied at the piece 4 merge"): R27's warning for a whole-outfit signature now exempts Future places (`isFuture`); piece 5's R18 added `LookItem.artNation` and a `CanLeak` row after row 5 (the leak would draw the same art as the claim's item), so §2.4's rows 6–8 are now 7–9; the Future places share one neutral hair and beard (`hair_{g}_neutral_future_{colour}`) instead of the art pipeline writing one drawing under each place's names.
