...
| Id | Decision |
|---|---|
| K1 | The Visitor window, its desktop icon, its auto-open (R12), `VisitorReachable` (R14) and the Visitor wiring warning are dropped. `TravellerPortraitView` is only the passport photo on the scanned page (no buttons, no highlights). `Looks.IsGarmentLayer` stays (every layer but `Body` and `Head` is a garment layer; the look menu lists garments). |
