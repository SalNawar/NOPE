...
- **`ShiftFlow` (TimeDesk.Domain, pure).** A decision table for closing: `OnClosing(travellerAtDesk)` → `FinishCurrent | CloseNow`. Tests encode the table. (The "no slot starts after closing" check lives in `DayOrchestrator`'s loop, which already owns slot sequencing.)
