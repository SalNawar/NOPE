...
- **`docs/ART_ASSET_LIST.md`:**
  ...
  - the skins of the UI this piece retires are marked "Retired in piece 7": `btn_back_to_office.png` ("< Office", §B) and `intercom_panel.png` with `intercom_button_normal/_hover.png` (§C). Only the unrebuilt hybrid scene keeps those objects, until it adopts the desk;
  - their replacements get rows: `btn_desk_normal/_hover/_pressed.png` (the taskbar's "< Desk", §B), and a new Tier-2 section I, the office overlay: the wheel's item and centre buttons, the speech bubble and the desk tooltip, built neutral (piece 6 decides whether the overlay is themed, §3.2).
- **`ArtDeliverables/TimeDesk/Reference/ART_ASSET_LIST.md`** is not edited: it is the export of the user's Google Doc that `PRODUCTION_PLAN.md` names as the art production's source, and it still carries the pre-piece-7 rows (the intercom "clicked to request documents", the intercom panel and buttons, "< Office"). `docs/ART_ASSET_LIST.md` supersedes it for every row above; the amendments reach the Google Doc and that export at the art sync.
...
- **The art list still asked for skins of retired UI and for none of their replacements** (fixed, docs only): `docs/ART_ASSET_LIST.md` marks `btn_back_to_office.png` and the intercom panel and button skins retired, and adds rows for the taskbar's "< Desk" and a section for the wheel's buttons, the speech bubble and the desk tooltip; §3.3 lists these amendments and records that the `ArtDeliverables` reference export is not edited.
...
## 11. Verification after the final review (2026-09-25)

Task 20 was run again in the branch's own Unity 6000.4.11f1 editor through the same temporary `-executeMethod` sessions (not committed), at `693cf55`, after the five fixes of §10. The builder, the generator and the content did not change, so nothing was rebuilt or committed; the sessions prove again that the committed scene is the builder's output, and new checks cover what §10 changed.

- EditMode suite (step 1): 562 passed. The two failures are the third-party `UnitySkills.Tests.Core.PerceptionSkillsTests.SceneSummarize_CountsObjectsCorrectly` and `UnitySkills.Tests.Core.SkillsModeManagerTests.Migration_RepeatLoad_IsIdempotent_NoDuplicateAuditEvent` (§9). No TimeDesk test failed. The suite holds one test more than in §9, `BoothRulesTests.TheFocusExit_IsNeverUp_WhileThePapersTakeInput_InAnyContext`. Offline Domain and Visuals run: 434/434.
- Content (step 2): Generate World ran twice and changed no file; Validate Content Library: no issues. With the validator's shared blueprint walk (§10), `MaxDocuments` is still 2 and `MaxRequestedDocuments` 1 for the shipped blueprints (read directly).
- Generation unchanged (step 3): the 60 slots are byte-identical to the baseline; `spoken` is true in the scene.
- Builder (step 4): two more builds of the committed scene logged no error and dump exactly Task 19's second build (1546 lines; TMP's `HideAndDontSave` `MeshFilter` left out, as in §9). The committed scene as loaded dumps the same, and the scene file is unchanged.
- Play-through (step 5; run seed 12349, travellers Archestrate, Caecilia, Beltani and Awil-Ninurta, as in §9): 115 `PASS` lines, no `FAIL`, and no warning or error. Steps 5.1–5.16 passed with §9's values: the scan took 1.50 s for 1.49 shift minutes, the passport's Coin of Issue logged "CURRENCY INCORRECT", and day 2's capital "Kaifeng (Bianjing)" logged "CAPITAL INCORRECT". Fourteen new lines check R38:
  - while the wheel is open the papers take no raycasts, and they are back in the raycast once it closes;
  - a paper held while another paper's scan finishes (`BoothCoordinator.Apply` mid-drag) keeps its collider off, and gets it back when released;
  - during the push-in and while focused the papers take no raycasts; back in the booth they take them again;
  - at the decision both papers slide away inert and out of the raycast, and they stay out for the 187 frames until they are destroyed;
  - a sliding paper still takes the press (step 5.6, as in §9);
  - at 2560 × 1080, the permit dropped at the desk's right end (u 1, v 1, at (2.90, −2.40)) shows in the focused view as a strip of 313 × 599 px at the lower left, with its collider off. A press on the strip's centre resolves to `FocusExitZone` and leaves focus, and back in the booth the permit takes input again. This is the check §10 left for a merge re-run.
- Other scenes (step 6), as in §9: Test_DayLoop played one case through its 6 era buttons with no error, and its log holds only the existing "no ShiftClockDriver wired" warning. OfficeScene_HybridArt played one full case on the no-desk path: the passport's window and icon at presentation, the one-shot permit request, "DEVIATION LOGGED — CURRENCY INCORRECT", then Deny. Its log held one "Desk scanner not wired" warning, the scene's existing wheel-or-transcript and ShiftClockDriver warnings, nothing about the brain and no error. The hybrid scene is unchanged.
- Screenshots (step 7), each read and judged against §6 step 7's checklist:
  - the twelve screenshots pass with §9's verdicts and art-pass notes; each matches §9's frame except for the wall clock's hands and the taskbar clock;
  - one more art-pass note: in `p7_bubble_1080` the three-item ring's right item ("Device") overlaps the CRT bezel's top-left corner by about 30 px. The label is unclipped, and the four-item ring in `p7_wheel_ask_1080` clears the CRT;
  - new, `p7_focus_2560`: pass. The permit's strip ("Permit", "trate") runs along the view's lower-left edge, clear of the glass. The desktop, the power button and the LED read as they do at 1080p. At 1920 × 1080, `p7_focus_1080` shows no paper at the left edge, as R38's reasoning says.
- Fixes made during verification: none. The sessions used §9's corrected scripts plus the R38 and document-count checks; a merge re-run (the plan's "Merging this branch", step 3) needs them as well.
- Hygiene: the temporary scripts are deleted. The csproj files, `ProjectSettings`, `RunConfig`, the TMP fallback atlas and the six `DeskFinish` materials were reverted (URP re-synced the materials when session C opened the hybrid scene, as in §9). The player's save was restored.
