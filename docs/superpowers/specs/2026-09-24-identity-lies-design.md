...
Run in the branch's own Unity 6000.4.11f1 editor through temporary `-executeMethod` scripts (not committed), at `efe385d`, after the implementation review's fixes:
...
- World check (seeds 12345 and 999 × days 1–3, plus 200 seeds × days 1–3): 0 problems. The same seed gives the same travellers, liars, homes, tells and paper values; a different seed differs. Claims, names, registered dates, roles and violator slots equal the baseline line for line. Every liar's true home is another of today's places; each tell carries the home's value and registers (mismatch on the claim's row, origin proof naming the home on its row, record proof for birth dates), and no third place's row registers anything. Honest travellers are fully consistent; violators are honest; records show the cover; every traveller is Male or Female. Sweep: liar rate 0.500 of 5081 eligible travellers (2543 liars among 6000 travellers); tells Currency 656, Language 598, BirthDate 672, Technology 617; 1097 cross-era liars on days 2–3; no NoPossibleLie warning, and no other warning or error while generating.
- Review fixes: the reworded `NoPossibleLie` warning keeps the "rolled a liar, but no other place" prefix the world check counts (0 hits). `NameRoster.BaseName` agreed, on all 6060 generated names, with an independent strip against every suffix `Roman` writes for 2..3999; no generated name carried a suffix on days 1–3 (no name pool ran out), so the suffix path rests on `NameRosterTests`, which pass in both runs below.
- EditMode suite: 345 passed. The one failure is the known third-party `UnitySkills.Tests.Core.PerceptionSkillsTests.SceneSummarize_CountsObjectsCorrectly` (scene-dependent, unrelated). Offline Domain run: 216/216.
- Scripted day-1 play-through on the committed OfficeScene (run seed 11; the builder is unchanged, so the scene was not rebuilt): the honest traveller was accepted without a citation; comparing a liar's Language tell with the true home's book row logged "… which belongs to Babylonia (Ancient)" and the denial was correct; the birth-date liar's Citizen Record showed the claimed origin and the cover date, comparing Born with Date of Birth logged "BIRTH DATE INCORRECT" and the denial was correct; denying a liar without evidence issued the unproven-denial citation; forcing closing time showed "Undocumented denials: 1". No warnings or errors; the Records wiring warning stayed silent. (The first run stopped at the results check because the script looked up `DayFlowUIController` among active objects only: its `DayFlowUI` host sits under the desktop `Canvas`, which `FocusOffice()` turns off at day end, while `ResultsPanel` is on the always-active `OfficeOverlayCanvas`. The script now includes inactive objects; no product change.)
