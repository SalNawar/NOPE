...
Branch `feat/history-facts` (worktree `E:\unity\NOPE-p5`), from `main` at `1a3e624`; Unity 6000.4.11f1 on the worktree only. Temporary automation (`_TimeDeskP5Baseline`, `_TimeDeskP5Automation`, `_TimeDeskP5PlaySmoke`, never committed; copies in the session scratchpad as `p5__TimeDeskP5Baseline.cs.txt`, `p5_final__TimeDeskP5Automation.cs.txt`, `p5_final__TimeDeskP5PlaySmoke.cs.txt`); reports `SCRATCH/p5_*.txt`.

**Offline, after every commit:** `compile_check_p5.py` 0 errors in every project; the reflection runner 539 passed, 0 failed (434 before piece 5; new: `ScoreKeyTests`, `ScoreRankingTests`, `DominanceTiersTests`, `InfluenceTests`, `NationLeaderTests`, `HistoryTests`, `CarriesTests`, `HistoryChecksTests`, `CultureCueTests`, `DayPlansTests`, `EndingRulesTests`; extended: `GatesTests`, `EffectOpsTests`, `FactTableTests`; `ForgeryTests` unchanged and green over the `TryFindOtherPlaceWith` extraction).

**Step 0, baseline before any piece-5 code** (`p5_baseline_cases.txt`, `p5_baseline_night.txt`): seeds 12345 and 999, days 1–3, 60 slots; one scripted day-1 night (perfect play, seed 12345).

**Session A, headless automation (49 checks, all PASS; `p5_automation_report.txt`):**
- Content: Generate World logs no error; a second run changes no file under `Assets/Data`; Validate Content Library reports no issues; `History/` holds 11 rule triggers, 11 SetFact effects, 8 leader effects; the library lists 48 places and 6 day plans, the 3 hand-authored triggers before the 3 unlocks and the 11 history triggers, the 7 hand-authored effects before the 11 rule effects and 8 leader effects, and the history lines with their ids; every nation has a Future place and a leader effect; Future places have no baselines.
- A broken source (a value shared by two places' rules, a 30-character fact, a rule naming an unknown place, a non-ASCII rule value, a dialog choice naming `Effect_History_roman_aljabr`) aborts with each error and writes nothing.
- Empty history changes nothing: days 1–3 for seeds 12345/999 are byte-identical to the step-0 dump (60 slots); the scripted night's news and scores equal the baseline (it held no "rising" line, so no intended difference showed).
- Leader with hysteresis from seeded scores: China 5 starts leading (announced "HISTORY: China now dominates the timeline. The Future belongs to Shanghai Megacity (Future).", cue `culture:china`, ranking saved); Japan 7 does not replace it (margin 2); Japan 7.5 does (announced; each leader's rule fired once); Japan falling to 1.5 (keep floor) hands the lead back to China; China falling to 1 with nobody above the floor leaves no leader, announced, and no cue.
- The Future on day 6: with China leading, exactly one Future place (`china_future`); the Currency book lists "Shanghai Megacity (Future)" with "Digital yuan (e-CNY)"; an honest Future traveller prints it on passport and permit; with no leader there is no Future place and, over 5 seeds, nobody claims or comes from the Future.
- A carry: accepting a day-4 liar (Mamluk Egypt as Northern Song Kaifeng) records "Carrier pigeon post"; next day Kaifeng's Technology is that value in the facts and the Index of Devices, the paper says "HISTORY: travellers brought Carrier pigeon post to Northern Song Kaifeng (Medieval).", and an honest Kaifeng traveller prints and answers it.
- News cap: a leader change plus 5 carries gives 3 templated lines, the leader's first; all 5 carries latch.
- Endings: day 15 gives nothing at an Immediate check and Retirement at the day boundary; with the Art total reached the Art epilogue replaces it (only at the boundary; not on day 14); fired wins at any moment.
- Saves: a `WorldState` with history and phase Home round-trips through JsonUtility unchanged; a version-2 JSON without `history`/`phase` loads empty and in the Office, and its first night names no Future place.
- Force leader (fresh run, day 5): the cue is on at once; after a night China is kept (ranking saved, no lost line) and day 6 holds `china_future` with the digital yuan and the cue; "No leader" clears the override, leader, cue and Future place; `DevToolsState.ResetAll` clears a set override.
- **Headless multi-day simulation** (§6 step 3; 50 seeds x 2 policies, 15 days, 10 travellers a day through `BuildToday`, `CaseFactory`, `ApplyVerdictImpacts`, `RecordCarry`, `EndingService`, `NightlyResolve`), every night checked with no failure: the saved ranking equals an independent re-sum of the scores; every leader change satisfies R2; at most one Future place, the leader's, and none without a leader; no traveller claims or comes from the Future without its leader; one carry record per accepted liar whose home Technology differs; every due carry latches and the next day's facts show the newest edit; each fired history rule latches its edits with its news line, once; no dominance line outside tomorrow's plan, no "rising" line, at most 3 templated history lines with the leader's first; Democratic Collapse never before the night of day 3; Immediate checks never return Retirement or an epilogue; the day-15 boundary always ends the run with Retirement or an epilogue. Determinism: the same seed and policy give byte-identical history JSON and per-day dumps. Report (`p5_sim_report.txt`):
  - perfect play: a leader in all 50 runs, first on day 4.1 on average (2–7); 1.66 leader changes a run (1–4); final leaders Greece 12, Iraq 12, Italy 9, Egypt 6, China 6, Germany 4, Japan 1; no carries (liars are denied); 2.56 history rules a run; no NoPossibleLie warning; day-15 totals Art 38.2 ± 6.7 (p65 41), Science 25 ± 6.8 (p65 26), Democracy 6.9 ± 8.2 (p65 9);
  - perfect play plus the first liar of each day: 13.8 carries a run; totals Art 33.3, Science 22.1, Democracy 5.2; no NoPossibleLie warning.
  - **Epilogue thresholds (R14)** set to the rounded 65th percentiles, Art 41, Science 26, Democracy 9 (first run with 45/32/15: Retirement 29, Science 8, Democracy 8, Art 5 of 50). With the final thresholds: perfect play ends Democracy 20, Science 16, Art 8, Retirement 6; the liar policy Retirement 18, Democracy 16, Science 12, Art 4.

**Session B, play mode on the committed OfficeScene (16 checks, all PASS; `p5_playsmoke_report.txt`):** a day-5 save at Home (phase Home) with China's influence 6: the Title offers Continue and it opens **Home**, not the Office; `RunManager.Sleep` runs the night: day 6 in the Office, China leading, the briefing shows the leader line and China's rule line ("Florentine printers set type the Chinese way"), the `culture:china` cue is live; slot 1 is the predicted honest traveller going home to Shanghai Megacity (Future), whose passport and permit print "Digital yuan (e-CNY)" (screen and passport window), and the Currency Ledger lists the Future row; accepting records no carry; closing the shift saves day 6 with phase Home and the leader; after destroying the run and reloading the Title (a fresh launch), Continue opens **Home** on day 6 with China leading; no error logged. Screenshots, read and judged: `SCRATCH/p5_briefing_day6.png` (the paper's leader and rule lines), `p5_future_traveller_booth.png` (the traveller's passport on the desk; the traveller is still piece 4's placeholder), `p5_future_passport.png` (banner "I request passage home to Shanghai Megacity (Future)."; Coin of Issue "Digital yuan (e-CNY)", Native Tongue "Mandarin (pinyin-first)"), `p5_future_books.png` (Currency Ledger page 5/7: "Shanghai Megacity (Future) — Digital yuan (e-CNY)" between Beijing (Modern) and Kofun Yamato), `p5_home_after_continue.png`. The player's save was restored afterwards.

**EditMode suite** (TestRunnerApi, final run): 667 passed, 2 failed, both third-party `UnitySkills`: the known `PerceptionSkillsTests.SceneSummarize_CountsObjectsCorrectly`, and `SkillsModeManagerTests.Migration_RepeatLoad_IsIdempotent_NoDuplicateAuditEvent`, which depends on EditorPrefs left by earlier runs (it passed in this session's first suite run, 668/1).

**Hygiene:** no builder change, so OfficeScene was not rebuilt; `OfficeScene_HybridArt.unity` untouched; Unity-touched files reverted (`*.csproj`, `ProjectSettings/EditorBuildSettings.asset`, `ProjectSettings/ShaderGraphSettings.asset`, the TMP fallback atlas, six `Assets/Art/Office/DeskFinish/Materials/*.mat` re-imported by the first open); the `_TimeDeskP5*` files and metas deleted.

**Departures and findings:**
- Found by the automation and fixed: `ContentLibrarySO` cached its lookups until a domain reload, so within one editor session after Generate World a lookup still read the old arrays (a new leader effect gave no cue; the dialog check called a generated effect "not listed"). `OnValidate` now resets the caches (`3d6a31d`).
- Found in self-review and fixed: `Carries.Make` could record a carry toward a claim that has no row today (`a1251bc`); claims always come from today's places, so this only hardens the rule.
- The Future era got its own small talk in world_source.json (the draft had none), so Future travellers always have something to say.
- §6 step 7's play-mode smoke became the scripted play-through above (§9).
- Not delivered here, by design: everything in §8 (after piece 4 merges).

**Open for Saleh:**
- With the R14 thresholds an epilogue is reached in 44 of 50 perfect runs (Retirement in 6): each epilogue alone is reachable in about a third of runs, but together they almost always replace Retirement. If Retirement should stay the common ending, raise the thresholds (the 80th percentiles are Art 44, Science 30, Democracy 13.5) or give the epilogues a gate.
- Day 1's four nations lead most runs (Greece, Iraq, Italy, Egypt: 39 of 50 final leaders under perfect play), as §7 predicted from the day plans.
