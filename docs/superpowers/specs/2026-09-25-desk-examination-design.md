...
## 11. The desk view (scope addition, 2026-09-25)

Saleh, verbatim: "when the player press on the mat the camera tilts forward so it is easier to see the desk". (His other ask, "when the player press on the stamp they can choose approve or deny", is the stamp tray, X13, already built.) The coordinator's decisions T1–T6, refined against the code:

| # | Decision | Why / refinement |
|---|---|---|
| T1 | **Toggle.** A left click on the mat that lands on no paper and no prop, with no paper held and nothing modal open, tilts the camera forward into the desk view; the same click in the desk view tilts back. With papers held, a click on the desk only puts them back (X-rules; it does not also tilt). A right-click on empty space in the desk view (nothing under the pointer that takes clicks, or the mat) also returns. **Escape order:** the frame, the wheel, the stamp tray, the held papers, then the desk view (X8: one press, one thing; each handler takes Escape only if its input was live since the previous frame). | The mat's click is a second catcher box (`Desk/ViewCatcher`) laid exactly like the desk catcher (the desk's clamp area, just under its plane, so papers and props win the raycast). The two catchers are never live together (BoothRules), so a click has one meaning. |
| T2 | **The pose.** Derived at bind from the art office's Cinemachine camera (its pose and lens) and the `DeskSurface` anchor's centre: moved `forward` metres along the view's flat forward and `rise` metres up, the yaw kept, pitched to aim at the mat's centre plus `pitch` degrees; the lens copied. Knobs in `DeskConfigSO.deskView` (`DeskViewTuning`: forward 0.6, rise 0.3, pitch −4, seconds 0.35; the pitch was 0 until the screenshots, §12). With Reduced Motion (piece 9's setting) the move is a cut. **A gameplay-owned `CinemachineCamera`** (`Office/DeskView/Camera`, built into `OfficeGameplay`) is raised above the art's office camera's priority while the desk view is on; the art camera's brain blends. The blend's time and style come from a `CinemachineCore.GetBlendOverride` hook that answers only for blends to or from the desk camera (the art brain's own default blend, 2 s, stays for everything else). The art scene is never edited and the art camera is never moved. With no art Cinemachine camera or no brain on the office camera the desk view is unavailable (its catcher never shows; one warning). | **Geometry, measured:** the art camera sits at (0, 2.16, −2.62), 10° down, vertical FOV 55°; the mat is 1.46 × 1.04 m centred at (0.13, 1.07, −0.52); the PC's front is 1.8 m left of the mat's centre and the stamp 1.2 m right. At 16:9 no pose lets the mat fill most of the screen with the PC's front and the stamp in view. The chosen pose (the "rise" is the refinement: moving up and forward looks more from above, which is what makes papers on the mat easier to see; moving down would flatten them) puts the mat in the screen's middle, papers lying on it about 1.6 times taller (measured 1.58, §13), the stamp, the scanner and the NEXT sign in view, the PC's front in view with its left edge cut at 16:9 (whole at 21:9), the traveller's head at the top. |
| T3 | **Everything follows the moving camera.** Held papers re-pose on a camera change (X2, every frame of the blend). The speech bubble and the wheel's ring stay on the screen when the traveller's head is above it: `OverlayProjection` places a point in front of the camera but outside the view at the screen's edge for them, below the case HUD's strips (tooltips still hide when their prop leaves the view). The stamp tray follows the stamp; hover outlines, the drag plane and its clamp, and the scanner's drop are world objects and camera rays. The claim tag and the office strip are overlay (unchanged). Clicks and drags work in both views. | The bubble would otherwise hide (its anchor, the head, is off-screen), and an answer asked through the intercom in the desk view would be lost. |
| T4 | **Returns.** When the next traveller is called (NEXT: the phase turns to TravellerAtDesk) and when the day closes (the report: the Newsletter phase), the view returns to normal, so the arrival and the report are seen. Opening the PC frame keeps the current view (the frame is an overlay; the camera stays). | BoothCoordinator: `SetPhase(TravellerAtDesk)` returns; `DeskViewAllowed` is false under a newsletter. |
| T5 | **The input table** (`BoothRules`): `BoothContext.DeskView`; `DeskViewToggleLive` = the props are live and no paper is held; `DeskViewReturnLive` (Escape and the right-click) = the desk view is on and the toggle is live; `DeskViewAllowed` = no newsletter. The desk view changes no other output. Tests first (`BoothRulesTests`). | Pure, like every other row of the table. The pose's arithmetic (the pitch that aims at the mat, the seconds under Reduced Motion) is engine-free in Visuals (`DeskViewPose`, tested: `DeskViewPoseTests`); the camera switch and the blend hook are engine code (`DeskView`, Office). |
| T6 | **Verified** in the persistent editor through job files: toggle in and out, Escape, the right-click, a drag to the scanner in the desk view, a held paper in the desk view (its slot, a desk click puts it back without tilting), the stamp tray in the desk view, the wheel and its answer in the bubble in the desk view, the frame keeping the view, the NEXT and closing returns, Reduced Motion's cut; screenshots of the desk view at 1920×1080, 2560×1080 and 1280×720, judged on whether the papers on the mat are easier to see. | §13 records the results. |

**Build (in `E:\unity\NOPE-art`, `feat/desk-exam`):**
- Domain: `BoothContext.DeskView`; `BoothInput.DeskViewToggleLive`, `DeskViewReturnLive`, `DeskViewAllowed`.
- Visuals: `DeskViewTuning` (the knobs), `DeskViewPose.Pitch(height, depth, tuning)` and `DeskViewPose.Seconds(tuning, reducedMotion)`.
- UI: `OverlayProjection.TryPlace(..., keepOnScreen)`; `OverlayCallout.keepOnScreen` (the bubble's is on); the wheel's ring keeps on screen.
- Office: `DeskView` (the toggle, the returns, the camera's priority, the blend hook; `Changed`); `DeskConfigSO.deskView`; `BoothCoordinator` (the context, the outputs, the NEXT return); `OfficeSceneBinder` (lays the view catcher, binds the desk view to the art camera and the mat).
- Builder: `Office/DeskView` (with its inactive `Camera` child, a `CinemachineCamera`), `Office/Desk/ViewCatcher`, the references; the bubble's `keepOnScreen`.
- `docs/FEATURES.md`: 3D input (the mat's click and the desk view), the Escape order, a new "Desk view" bullet.

## 12. Build notes and deviations (the code wins)

Built on `feat/desk-exam` from `origin/main` (`d5844d0`) in `E:\unity\NOPE-art`; `origin/main` at `eac05f9` (the art side's new office) merged in before the final verification. Where the build differs from §3, §11 or the plan, with the reason:

**Plan order and process**
- Knobs land with their first reader: `DeskConfigSO.face` in the paper-face commit, `examine`, `examineTint` and `rowHoverTint` in the examine commit, `stampTrayOffset` in the stamp commit, `deskView` in the desk view's commit (the plan put the first four in Task 9). No commit carries an unread knob.
- `DocumentWindowController.Refresh()` lands in the examine commit, with its first caller (`InvestigationUIController.Sighted`), not in the refactor commit.
- Small edits to existing files were made with an EOL-preserving editor and checked by a mixed-line-ending scan before every commit (`SCRATCH/p10/eolcheck.py`); larger ones went through `subs.py` scripts in `SCRATCH/p10/`.
- The generated UI table lives in `Assets/Data/World/Culture/Strings_en.asset` (piece 6), not in `ContentLibrary_Main`; Generate World changed only that file (`desk.scanHint`).
- The domain commit of the desk view (`3a347d9`) has the coordinator pass `false` for the desk view until the view lands in the next commit, so each commit compiles.

**Domain and Visuals**
- `DeskPapers` has no `SlotOf`: nothing outside the rule reads a slot (the controller takes it from `HoldResult`), so it was removed as a member without a caller (`28df8b7`). The tests read slots through `HoldResult`.
- **The cover rule (`HeldCover`, added after the merge, `8ef7261`).** In the new office the permit, handed over while the passport was held, landed under the held passport and could not be clicked. A paper now prefers the slot on its own side unless that slot would hide a paper still on the desk and the other would not (`HeldCover.PreferRight`), and a paper handed over while papers are held lands on the first spawn spot no held paper covers (`HeldCover.LandingSlot`; under them only when both slots are full). Tested in `DeskPapersTests`; `docs/FEATURES.md` says it.
- `DocumentReveal.Begin` returns true when the call started the clock, so a sighting redraws both surfaces only then.
- `SpeechQueue.Hold(true)` also brings a line already past its fully-shown point back to it, so its whole hold (or minimum) runs from the release, as §2.4 says.
- `PaperFace`: a row's hit area is its label's top to its value's bottom; the gap under it belongs to no row (`RowAt` gives -1 there).
- The face's label and value shares are 0.42 and 0.50 (§3.3 said 0.36 and 0.52), and the label ink is darker: the 720p screenshots showed the labels at about 10 px in a light grey, readable but strained. TMP's auto-size now rounds labels and values to the same pixel size (15.6 px at 720p, 23.4 px at 1080p); the label is told from its value by its lighter ink and its place above. Values stay at least 14 px at 720p; the capacity stays 6.
- `BoothRules` (§11 T5): one output, `DeskViewReturnLive`, covers both Escape and the right-click (they have the same condition).

**Glue**
- The compare refactor keeps every PC path, with one refinement stated in its commit: "the same row again clears" is told by the value's key, so it also clears after a page flip rebuilt the row (it used to pair a value with itself).
- `PcFrame.RightEdgePixels` reads the bezel as the glass's parent (no new serialized field); `SetExamineHole` closes the hole by moving it off-screen at zero size.
- `PaperExaminer` also answers `RightOfCentre(world)`, `SlotCovers` and `HeldCovers` (the desk asks which side a paper lies on and what a slot would hide). Papers that do not fit beside the open frame stay at their office slots behind it and get no hole, so the frame's surround takes the click (inert, as X10 says). **It also re-poses when the open frame's right edge changes** (`797bd70`): the overlay's canvas scaler follows a new screen size a frame after the examiner saw it, so at 1280×720 the papers kept the previous size's edge and stayed behind the frame.
- `DeskDocument` exposes `FieldRow(int)`, `RowCount`, `IsExamined`; its paper quad and examine material are serialized (`paperQuad`, `examineMaterial`); `SetExamined(bool)` reads the tints from the config it was bound with.
- `DeskController`'s old `_held` (the dragged paper) is renamed `_dragged`, since "held" now means in the hand.
- The same-frame Escape guard is also in `OfficeViewController` (the frame), which the plan's file map did not list; X8 asks it of each handler. The desk view has its own (§11 T1).
- `TravellerWheel` tags a line with its index among the lines said to the bubble this case; the investigation finds the clicked line's transcript index by reference for its pick key. The highlight is `BubbleHighlightNow` (the line on show); `ClickLine()` is SpeechBubbleInput's entry.
- `SpeechBubbleInput` releases the hold when the panel hides (an inactive panel never receives the pointer's exit).
- `StampTray` closes only on a click on its catcher itself, not on the panel's background.
- `OfficeCaseHud` has a `root` (the HUD) and a `claimRoot` (the tag hides with no claim).
- The overlay's verdict text is a child of its strip (it was a sibling on the desktop).
- The builder lays the template's title, photo frame and first row from `Desk_Default.face` at build time; at runtime each paper re-lays its title, photo position and rows from the same knobs (the photo frame's size stays the built one: re-run the builder after changing the face, as for the capacity check).
- The capacity check counts `DocumentTemplateSO.fieldSpecs`.
- **The day-1 scan note** (the new, longer text) shrank to one tiny line in the floating note's one-line box; the builder gives the scan note a two-line box (0.78 × 0.13 m, `1241364`).

**The desk view (§11), as built and measured**
- The coordinator's "down" knob is `rise` with the sign turned: the pose moves up (0.3 m) and forward (0.6 m); moving down would flatten the papers on the mat.
- The pitch knob defaults to **−4°** (4° above aiming at the mat's centre), not 0: aimed at the centre, the view's bottom showed the floor under the desk's front edge (`8e2a9bc`). The measured desk view pitches 38.9° down (the normal view 10.2°) from (0, 2.46, −2.02) (the art camera at (0, 2.16, −2.62)).
- The speech bubble and the wheel's ring, held on the screen while the traveller's head is above it, first covered the claim tag; they now keep the case HUD's band clear (`OverlayCallout.topInset`, `TravellerWheel.ringTopInset`, 152 reference px from the builder's strip layout).
- The camera never moves by our code: the art camera's brain blends to the desk camera, and a `CinemachineCore.GetBlendOverride` hook times only the blends to or from it (EaseInOut 0.35 s; a cut under Reduced Motion). The art camera's pose and its brain's own 2 s default blend are untouched.
- The day-1 scan note and wheel note are faced to the viewer at bind only; in the desk view they are seen from 0.67 m further forward and 0.3 m higher (a few degrees off), which the screenshots show as harmless.
- Not done: a held paper during the 0.35 s blend is re-posed in `LateUpdate`, which may run before the brain's; only a paper picked up mid-blend can trail by a frame (no paper is held when the view turns: the toggle needs none held, and NEXT and closing clear the desk).

## 13. Verification record (2026-09-25, `E:\unity\NOPE-art`, branch `feat/desk-exam`, Unity 6000.4.11f1)

Built from `d5844d0` as planned, then `origin/main` (`eac05f9`: the art side's new office from art `58bda15`, art brief v2.1, the leak-item content change, and the contract fixes for the new desk) was merged in before the final verification (`7bb9a6f`, no conflict), at the coordinator's request. The office used for the final verification is that new art office. The desk view (§11) was added after the merge at Saleh's request and verified in the same office. Temporary `_TimeDeskP10*` scripts (never committed; reports and screenshots in the scratchpad as `p10_*`). The early runs launched a Unity editor per check; after Saleh asked for one editor, every later run went through the persistent editor's job runner (`Library/ClaudeJobs`), and no script calls `EditorApplication.Exit`. The final pass (`p10_pass3`) and play-through (`p10_play4`) ran on exactly the committed code.

**Offline** (`compile_check_art.py`: 0 errors in every project, no warning in a touched file; the reflection runner on `cc_art`): `passed 946` at `d5844d0`; `passed 1047, failed 0` at the end: +10 `DocumentRowsTests`, +13 `ComparePairTests`, +27 `DeskPapersTests` (holds, slots, click routing, the cover rule), +10 `BoothRulesTests` (5 for the stamp tray and held papers, 5 for the desk view), +15 `ExamineLayoutTests`, +7 `PaperFaceTests`, +5 `RevealClockTests`, +8 `SpeechQueueTests`, +6 `DeskViewPoseTests`. Each Domain and Visuals task wrote its tests first and saw them fail (to compile, or the changed default).

**Task 0.** The font spike on a world `TextMeshPro` (in a fresh scene, through `TextFlip.Write` with the runtime fonts from `RuntimeFonts`, as `TranslationPresenter` resolves them): hieroglyphs (`seguihis.ttf`), Arabic shaped right to left (`tahoma.ttf`) and Han (`msyh.ttc`) all draw with no missing glyph, and the font swaps to the script's runtime font (`p10_world_tongues.png`); the tracked TMP assets are unchanged. The generation baseline (seeds 12345 and 999 × days 1–6, day 6 also under the Japan and Egypt leaders; piece 9's columns; 194 travellers) equals piece 9's final dump byte for byte.

**Determinism** (X23). On the branch before the merge the dump is byte-identical to the `d5844d0` baseline. After the merge it differs from that baseline in exactly three travellers (seed 999: Otama and Onatsu of Tokugawa Edo wear the kazuki veil; Lenio of Ottoman Ioannina the silver chest chains), all main's leak-item change (`68fbbe6`); every later dump (after the cover rule and after the desk view) is identical to the merged one. Piece 10 draws no random number and changes no generation.

**The Unity pass** (`p10_pass_report.txt`): Generate World twice (the second changes no file); Validate Content Library: no error, no warning; case generation twice gives the same 194 travellers; Build Office UI three times: no error or warning, a rebuild of a fresh build holds the same objects and values, and a further rebuild changes nothing (semantic dump, 3639 lines); every new part exists and is wired (the paper template's rows, highlight and examine material, the desk catcher and examiner, the frame's second hole, the bubble's input, the stamp tray and the stamp's click, the office case HUD and strip, the verdict line and citation on the overlay and none left on the desktop, the overlay's order, the mat's catcher, the desk view and its inactive Cinemachine camera at priority 0, the bubble's and the ring's HUD inset); the scene contract resolves (20 fallbacks, 3 defaults, none missing); the art office's scene file is byte-unchanged and not dirtied; player scripts compile (44 assemblies); **the EditMode suite: 1176 passed, 1 failed** (the known `UnitySkills.Tests.Core.PerceptionSkillsTests.SceneSummarize_CountsObjectsCorrectly`).

**The play-through** (`p10_play_report.txt`, seed 12345, Title → New Run, days 1–3 in the art office with the gameplay layer, Home → Sleep between days; day 2 without translators, day 3 with the Near East and Mediterranean Papers and Speech translators): **197 checks passed, 0 failed.** The log shows only the binder's known anchor warning (once per office load). Days 2 and 3 take the travellers as the history made on day 1 sends them, and run each check on the first traveller that suits it.
- **Arrival** (Lysimache): the claim tag reads the PC banner's text; the passport lands with 4 rows (the scanned copy's, in order) and the photo; day 1 in English; the day-1 note reads the new text (`p10_arrival_{1080,2560,720}`).
- **Examine one and two:** a click lifts the passport into the slot on its side; each held paper projects onto its office slot within 2 px at 1920×1080, 2560×1080 and 1280×720; the slots clear the head, the bubble, the PC and the stamp; every text is inside its box; the value lines are 23.4 px at 1080 and **15.6 px at 720p** (labels the same); the row under the pointer tints; two held papers never overlap (`p10_examine_one_*`, `p10_examine_two_*`); a third paper sends back the one held longest to where it lay (`p10_examine_evict`).
- **Compare at the desk:** passport Coin of Issue against permit Bond Currency: MATCH, nothing logged, both rows lit, the PC bar the same (`p10_compare_papers`); the same row again clears. Theano's answer in the bubble (held past 4 s by hovering) against her paper's Coin of Issue: MISMATCH, nothing logged (`p10_bubble_pick`).
- **Put back** each way (right-click, the title, the desk, Escape), a prop's click keeps the papers; **drag-out** onto the scanner scans and returns the paper to its spot; a drag onto the busy scanner slides back.
- **Wheel:** held papers dip under the ring and take no input at the three sizes (`p10_wheel_dip_*`); Look fills the office strip (`p10_look_strip`); a bubble pick keeps the wheel open; **Escape order** (the wheel, then the papers; the frame, then the papers).
- **Stamp tray** above the stamp at the three sizes (`p10_stamp_tray_*`); everything else inert while it is open; accepting the honest Lysimache pays; deciding while holding two papers and while a paper scans clears the desk; an unproven denial shows the citation over the office with the clock paused (`p10_citation_office`); Acknowledge resumes.
- **Frame region** (Theano): both papers right of the open frame, stacked at 1080 and 720, side by side at 2560 (`p10_frame_region_*`); the Currency Ledger's claim row and the paper's Coin of Issue log DEVIATION LOGGED, listed in the Deviation Report (`p10_frame_deviation`); a click beside the papers closes the frame and they return to their slots; **cross-surface pending**: a book row picked in the frame waits in the office strip and pairs with a desk row (ALREADY DOCUMENTED, `p10_cross_pending`); the same field on the scanned copy and the paper clears. **The PC's paths** after the refactor: a transcript answer against the book row (MATCH), the Citizen Record's born row against the scanned copy (MATCH), the scanned copy against the book row (ALREADY DOCUMENTED).
- **Day 2, untranslated:** Shat-Aya's Akkadian values in the cuneiform runtime font with labels, name and date in English (`p10_untranslated_paper`); a pick puts the placeholder in the strip (`p10_placeholder_strip`); Tuya's hieroglyph passport at 720p drawn (value lines 12.4–18.6 px, `p10_hieroglyphs_720`, as faint as piece 9's); Socrates' untranslated spoken tell against the book row and Awil-Ninurta's untranslated paper tell beside the frame both log in English (`p10_untranslated_proof`).
- **Day 3, translated (X25):** Arib al-Ma'muniyya's first examine flips her values into English by 1.42 s (the longest row's flip 1.33 s; `p10_flip_mid`, `p10_flip_done`), and the later scan shows English at once; Tuquztimur's permit scanned with the frame closed waits untranslated, the frame's opening starts its flip (`p10_flip_frame`), examining it afterwards shows English, and a scan finishing with the frame open flips at once; Ambrogio's row click mid-flip finishes both surfaces; with reduced motion Quintus's first sighting shows English at once.
- **The desk view (§11 T6),** with Senenmut at the desk: the mat's click tilts into the desk view and the camera reaches its pose; **the passport lying on the mat is 1.58× taller on the screen (68 → 108 px at 1080) and sits in the screen's middle (240 → 508 px from the bottom)**; the stamp, the scanner and the NEXT sign are in view, and the PC's front with its left edge cut at 16:9 (whole at 21:9) (`p10_desk_view_{1080,2560,720}`); the same click tilts back onto the art camera's pose, which never moved; Escape returns; a right-click on empty space returns; a paper held in the desk view projects onto its slot within 2 px and the mat is off while it is held (`p10_desk_view_held`); a click on the desk then only puts it back; Escape puts the paper back first, then returns; opening the PC keeps the desk view and Escape closes the frame first; the wheel opens through the intercom with its ring on the screen below the HUD strips (`p10_desk_view_wheel`), Escape closes it first; an answer shows in the bubble below the HUD strips (`p10_desk_view_bubble`); the stamp tray opens above the stamp and Escape closes it first (`p10_desk_view_tray`); a drag onto the scanner scans. The desk view entered between travellers returns when NEXT calls Senenmut, so his arrival is seen from the normal view; entered before closing, it returns under the shift report; with reduced motion it cuts both ways.
- **Judged by eye:** held papers readable at 720p (labels and values at 15.6 px); the slots clear the traveller's face, the bubble, the PC and the stamp in the new office; the held paper in the right slot hides the left of the NEXT sign, which is only needed after the decision (when the papers have left); the desk view makes the papers on the mat easier to see (bigger, central, seen more from above), though their text still needs the examine to be read at 1280×720.

**Notes from the verification (not changed by this piece):**
- **The day-1 wheel note** ("Click the traveller to talk and ask for papers.") is small and low-contrast over the pale morning crowds of the new office. It is the office move's `FloatingNote` (piece 10 does not touch it). Lever: the note's box and size as for the scan note, and a darker backing or outline.
- **The day-1 scan note** now wraps to two lines at its old size, but at the scanner stand-in's default spot it is partly behind the new desk's pen pot. It follows the scanner anchor; an `Anchor_Scanner` in the art office (the contract's default pose is in use) fixes both.

**Knob values at the end** (`Desk_Default.asset` keeps its values; the new knobs are the class defaults): `face`: side margin 0.07, title top 0.03, title height 0.08, rows top 0.13, row pitch 0.125, label share 0.42, value share 0.50, bottom margin 0.04, photo top 0.13, photo height 0.25, photo gap 0.03; `examine`: office height 0.44, office centre x 0.2, office bottom 0.02, wheel dip 0.2, frame height 0.62, frame min height 0.3, margin 0.02, gap 0.03, distance 0.6 m, near margin 1.5, desk margin 0.8, roll 0, seconds 0.18; `examineTint` white; `rowHoverTint` black at 6%; `stampTrayOffset` (0, 140); `deskView`: forward 0.6 m, rise 0.3 m, pitch −4°, seconds 0.35.

**Screenshots worth showing Saleh** (scratchpad): `p10_desk_view_1080.png`, `p10_desk_view_held.png`, `p10_desk_view_bubble.png`, `p10_examine_two_1080.png`, `p10_compare_papers.png`, `p10_bubble_pick.png`, `p10_frame_region_1080.png`, `p10_frame_region_720.png`, `p10_frame_deviation.png`, `p10_stamp_tray_1080.png`, `p10_citation_office.png`, `p10_untranslated_paper.png`, `p10_hieroglyphs_720.png`, `p10_flip_mid.png`, `p10_examine_one_720.png`.

## 14. Self-review and intent audit

**Against Saleh's words.**
- *"the dialogue of the person saying where they want to go and giving the documents to happen in the office not in the pc"*: the traveller says the claim in the bubble and hands the passport onto the desk (as before); what was PC-only is now in the office: the claim with the name and the role (the claim tag, the PC banner's own text, always English), every field of every paper (the paper's whole face), the answers as evidence (the bubble), the compare bar (the office strip), the verdict (the stamp tray), the verdict line and the citation. Nothing about who the traveller is, where they go or what their papers say needs the PC.
- *"the player will use the cursor to pick the documents examine them like in paper please against each other"*: a click lifts a paper into the hand; two sit side by side; a row of one picks against a row of the other (MATCH or MISMATCH, both rows lit), against the bubble's answer or against a garment. Papers are still dragged on the desk as before.
- *"then they can scan them one by one"*: scanning is still a desk drop, one at a time; a held paper can be dragged straight onto the scanner.
- *"and perform more complex things in the computer if they want"*: every PC function is kept (the compare refactor is behaviour-preserving and was re-verified on the book, transcript, Citizen Records and scanned-copy paths). **The one place the office still needs the PC** is a *proven* denial: a proof needs a truth row (a book or a Citizen Record), and those stay on the PC (E1's stated deviation). A player who never opens the PC can still decide every case at the stamp, but denying a liar that way is an unproven denial with a citation. The office makes this as short as possible (a held paper sits beside the open frame, so the book row and the paper row pair in one view; a pick waits across the frame's opening and closing). Q1 (desk books) is the lever if Saleh wants proofs without the PC.
- *"when the player press on the mat the camera tilts forward so it is easier to see the desk"*: a left click on the mat tilts the camera forward (and a little up and in) over the desk and the same click tilts it back; the papers on the mat come to the screen's middle, 1.6 times taller. To read a paper's rows the player still picks it up (the examine); the desk view is for seeing the desk and its papers, dragging them and reaching the scanner and the stamp. *"when the player press on the stamp they can choose approve or deny"*: the stamp tray (X13).

**Redo and override checks.**
- No old path still does the old thing: the scan no longer starts the written reveal (only a sighting does); the click no longer brings a paper to the top (it examines; a put-back lands on top); the desktop no longer builds the verdict line or the citation slip; the holder line and its field are gone; `CompareController`'s old `Select(string, string, Image, CompareEvidence)` and every inline evidence build are gone (grep: `CompareEvidence.For*`/`From*` only in `EvidencePicks` and Domain).
- The PC's Accept and Deny still decide (Aemilia accepted on the PC, Theano denied on the PC); the stamp tray decides through the same `Decide`, so the evidence gate, scoring and the ledger have one path.
- The desk view overrides nothing of the art side's: the art camera's pose, its brain's default blend and the art scene file are untouched (checked: the art camera never moved; the scene file byte-unchanged); the blend hook answers only for the desk camera's blends and hands every other blend to the hook before it or the brain.
- A click on the desk has one meaning at a time: the desk catcher (papers held) and the mat (none held) are never live together (tested over every context).

**No-code-path checks** (each new behaviour is reachable in the built scene, checked by the builder automation and the play-through): the examiner, the desk catcher, the frame's examine hole, the bubble's button and input, the stamp tray (the stamp's click), the office case HUD and strip, the overlay verdict and citation, the mat's catcher, the desk view's camera (bound at load, live by the input table), and the bubble's and the ring's HUD inset. Where the art office lacks a stamp (no `Stamp` anchor or fallback) the binder deactivates the stamp's box and the tray cannot open; the PC's Accept and Deny remain. Where it lacks a Cinemachine camera with a brain, the binder warns once and the mat never shows.

**Self-review findings fixed during the build**: the row labels were too faint at 720p (fixed: larger, darker); `DeskPapers.SlotOf` had no caller (removed); a permit handed over while the passport was held landed under it in the new office (the cover rule); held papers stayed behind the frame at 720p (the frame edge re-pose); the scan note shrank to one tiny line (the two-line box); in the desk view the bubble and the ring covered the claim tag (the HUD inset) and the floor showed under the desk (the pitch knob); in-flight papers (0.18 s) can briefly cover a click target (by design; the automation waits for landings).

**Open items for Saleh:** Q1–Q10 (§10) stand; add: should the desk view be allowed to lose the PC's front for bigger papers (forward 0.9 m makes them about twice as tall as in the normal view, by projection, but the PC's glass leaves a 16:9 screen), and should the day-1 wheel note get a backing (see the notes above).
