...
- **As built (2026-09-26, branch `redesign/p06-present-accounts`, main `83a43a6` merged in; phase 5 was not built, so TC-101/230 print through phase 4's desk forms and the scanned copy follows when phase 5 lands):**
  - The present: `world_source.json` `present` (name, year, facts, wardrobe; its Culture derived as a place's) → `PresentContent` on the library; `ContentLibrarySO.BuildPresent(history)` (`Present.Choose`: the leader's Future place, else the neutral present; builds one place) → `TodaysWorld.Present`; `BuildToday` adds its row with `Present.AddRow` (never twice). `Present.NeutralNationId` = "neutral". The wallet reads the present's currency (R4-018).
  ...
  - The clerk: phase 25's `IClerkAccountSource` stays the one source; `AccountRecords.Clerk(profile, Account.ExtractRows(...))` puts the clerk's own account in Citizen Records (`GameManager.BuildRegistry`), no row evidence; the clerk's Citizen ID is reserved in the day's numbers. **Phase 13**'s debt shows there with no change.
