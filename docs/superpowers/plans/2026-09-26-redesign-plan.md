...
- **As built (2026-09-26, ahead of 18, 20 and 6-12; branch `redesign/p21-steps`):**
  ...
  - A step's categories are kept by name, so a data-only set names the categories phases 6-9 append (AccountStatus, TransponderId, TransponderClass, Debt, Credit, Funds, PolicyNo, Employer, Term, Wage, WaiverNo) with nothing to renumber; a name no category has yet counts nothing. A set may be marked `dataOnly`: its forms and categories go unchecked, and the validator warns once a blueprint makes its kind. **Phases 6, 8 and 9** drop the mark of their kind's set and check its forms and categories against what they add. A step may start on a later day (`fromDay`, the traveller-types spec's "from day N").
