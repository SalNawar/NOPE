...
- **Depends on:** 3, 5.
- **As built (2026-09-26, branch `redesign/p06-present-accounts`, main `a733683` merged in: TC-101/230 print through phase 4's desk forms and phase 5's scanned copies):**
...
- **As built (2026-09-26, ahead of 18, 20 and 7-12, with phase 6 merged in; branch `redesign/p21-steps`):**
  - Domain: `CaseSteps` (Resolve, Evaluate, Categories, Target, Classify), `CaseProgress` with `StepPaper` (the hand-set ticks live in it, not in a separate dictionary) and `StepSets` (the source shape, Parse, Problems, DataOnlyInPlay). A step's jump is `link` (`StepLink`: Tab, PrimaryName, FirstUncheckedField, CostumeClaimed) with `tab` (`AppTab`), not phase 18's `LinkTarget`, which is not on `main` yet; the JSON names a link in PascalCase like every other enum value (`"jump": { "link": "PrimaryName" }`).
  - A step's categories are kept by name, so a data-only set names the categories phases 6-9 append (AccountStatus, TransponderId, TransponderClass, Debt, Credit, Funds, PolicyNo, Employer, Term, Wage, WaiverNo) with nothing to renumber; a name no category has yet counts nothing. A set may be marked `dataOnly`: its forms and categories go unchecked, and the validator warns once a blueprint makes its kind. Phase 6 put rich tourists in play before this phase merged, so the RichTourist set lost its mark in the merge (its forms, TC-101 and TC-230, and its categories are checked; on day 1 it lists rules, papers, read, identity, class and transponder). **Phases 8 and 9** drop the mark of their kind's set (PoorTourist, Labourer) and check its forms and categories against what they add. A step may start on a later day (`fromDay`, the traveller-types spec's "from day N").
  - The parts: a paper step counts the case's papers of its forms (all when it names none); a Requested step with forms is one request group (any of them ticks it: phase 8's proof of means and its missing-form replies); a step with no parts in a case is not listed. A pair counts only when both sides are of one category; categories are derived only against the reference (the papers' categories a book holds) or for answers (today's question categories). Against the P spec's default table: `identity` also counts the CitizenId (it exists since phase 3), and `answers` checks against any truth (a birth-date answer's truth is Records).
  - The labels are short ("Name, number and birth date in Records"), two lines of the 272 u sidebar at 13-16 u; the spec's sentences did not fit. The Steps section takes the sidebar's top 62 %, Pinned and Recent the rest.
  - The UI: `StepsPanel` and `StepRowView`. A paper shown in Documents and the Rules tab count only while the player looks at the PC (`MonitorScreen.IsInteractive` and the app showing), so a scan that shows a paper while the frame is closed reads nothing. A Records lookup of a name or number counts as a record viewed (a step's jump looks the primary record up, so it counts too). A hint toast shows without Open.
  - Settings' Investigation section (Steps shown / Steps hidden) sits between Desktop and Keyboard; Keyboard and the note moved down (the note at 15 u). **Phases 18 and 20** add their rows there.
  - **Phase 20** binds Ctrl+Shift+S to `StepsPanel.Toggle()`. **Phase 18**: the jumps call `InvestigationApp.ShowTab`, `CitizenRecordsWindowController.Lookup` and `ReferenceView.ShowBook`; with two panes they go to the active pane, and `StepTarget` can become a `LinkTarget`.
  - Readability: at 720p the step labels are as small as the rest of the sidebar; the epic's readability audit should look at it.
