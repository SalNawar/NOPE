...
- **As built (2026-09-26, ahead of 4, 13, 17, 20 and 24; branch `redesign/p25-apps`):**
  ...
  - Mail's Times issue is its link for past days: `MailSources.News` returns null for them. **Phase 24** returns the archived issue's lines (`WorldState.newsArchive`) and points the "internet" app at the browser; Mail's News link then opens it.
  ...
  - The authored mail is a top-level `mail` section of `world_source.json` (not `pc.mail`, so it does not collide with phase 24's `pc` block); it can move under `pc` once both are on `main`.
