...
Four steps. Everything referred to here is in `CHARACTER_ART_BRIEF_v2.md` (the brief, v2.1: revised for the 3D office).
...
- 40 places, each with a man's and a woman's look, plus the Future, plus 10 historical characters drawn whole later.
...
- Clean, simple 2D style: soft, friendly faces on realistic adult proportions, flat colours with one soft shade tone (the same hue, a little darker), almost no texture, clean dark-brown outlines.
...
- Premade characters (Senenmut, Socrates and the others) are made in a second Project, "Time Sorter Premades", with its own instructions (Block P), because they are real people drawn whole in their real hair colours (brief, section 11).
