...
- **Wired (drop-in):** the bubble and its tail are by-name slots: `Assets/Art/UI/Resources/Office/speech_bubble.png` (sliced at a quarter of its shorter side by the importer, so keep the rounded corners inside the outer quarter and no empty margin round the edge) and `Assets/Art/UI/Resources/Office/speech_bubble_tail.png` (its top meets the bubble's bottom edge). Until they exist the game draws the plain panel and no tail. Ask ChatGPT for the square 1024 x 1024 and Claude scales it; keep the originals in `ArtDeliverables/TimeDesk/UI/Raw/`.
...
- **Today** every document uses one placeholder texture, `Assets/Art/Office/Placeholder/paper.png` (150 x 200, cream). **Wired (drop-in):** a face per form kind, `Assets/Art/UI/Resources/Forms/paper_<form number>.png` (`paper_tc610` for TC-610), else the plain agency face `Assets/Art/UI/Resources/Forms/paper_agency.png`, else the placeholder; the photo frame's art (`Forms/photo_frame.png`, 4:5 with a transparent window) is drawn over the photo; Temporal Customs' seal (`Forms/agency_seal.png`) is printed by the game from its own file at 10 %, so the faces still carry no seal; the verdict's ink marks (`Forms/stamp_accept.png`, `stamp_deny.png`) land in the form's stamp area.
...
- **Costume Guide cover** (`Assets/Art/UI/Resources/Investigation/refbook_cover_culture.png`, **400 x 560**, next to and like `refbook_cover_currency.png` and the other reference-book covers; drop-in): a closed book cover with a simple garment motif (a hat and a collar), **no title**. The game shows the book's name, and the cover at the top of the book's page in the Investigation app. Ask ChatGPT for the tall size and Claude trims and scales it.
...
- **Traveller wheel icons** (piece 8; unchanged; `Assets/Art/UI/Resources/WheelIcons/wheel_<kind>.png`, **64 x 64**; the importer makes them sprites and the game loads them by name, so no code change): one per kind of wheel choice, shown at 28 px at the left of each choice on a dark blue button. `wheel_request` (a sheet of paper or an open hand: the desk asks for something), `wheel_question` (a speech balloon), `wheel_look` (an eye), `wheel_dialog` (two speech balloons), `wheel_back` (an arrow pointing left), `wheel_normal` (a small dot: a reply inside a conversation). Bold, simple silhouettes, **white on a transparent background** (the game may tint them, rule 5), readable when tiny. No letters and no character shapes: no question mark, no exclamation mark. Until they exist the game draws white placeholder glyphs of the same shapes at runtime. Ask ChatGPT for a square image and Claude scales it.
