...
*2026-09-24 · decisions made by Claude under Saleh's instruction "go with all pieces, don't stop" (2026-09-24), open to his review · branch `feat/history-facts` (stacked on `feat/characters`, piece 4)*
...
Line numbers refer to `efe385d` on `feat/identity-lies` (piece 2 complete: `efe385d` verified, `feat/identity-lies` ends at `dc676cd`). Pieces 3 (`feat/dialog-questions`, spec committed there at `fbf4905` as `docs/superpowers/specs/2026-09-24-dialog-questions-design.md`, code not yet written) and 4 (`feat/characters`, drafted in parallel in the session scratchpad `specs/2026-09-24-characters-design.md`) land before this piece and move these lines, so the piece-5 plan re-reads every anchor. This spec builds on:
...
- Looks: a Future traveller wears their Future place's culture-shaped outfit (piece 4) and the shared neutral Future hair (R18). No premade claims or comes from the Future (R22).
...
Each era has weight 1; tell count 1; the tell channels copy day 3's (piece 3 Papers + Answer, piece 4 Appearance); the premade pool and chance copy day 3's, with no forced premade (R22).
...
- **TimeDesk.Domain** (`Assets/Scripts/Domain`, pure, EditMode-tested):
  ...
  - changed: `Gates.cs` and `EffectOps.cs` (piece 3), `LookData.cs`, `Looks.cs` and `LookKeys.cs` (piece 4), `FactTable.cs` (`MaxValueLength`, `TryFindOtherPlaceWith`, doc), `Forgery.cs` (its uniqueness loop calls `TryFindOtherPlaceWith`).
- **Assembly-CSharp:**
...
- **Assembly-CSharp-Editor:** `WorldContentGenerator.cs` (including piece 3's dialog-effect and line-length checks and piece 4's wardrobe item data), `ContentLibraryValidator.cs`. `OfficeSceneUIBuilder.cs` is unchanged (its `GetDayPlan(1)` keeps its result), so the scene is not rebuilt.
...
Line endings in the worktree (`core.autocrlf=true`; preserve per file, `SCRATCH/subs.py`; checked by byte count at `fbf4905`, whose code equals `efe385d`):
- **CRLF:** `ContentLibrarySO.cs`, `CaseFactory.cs`, `GameManager.cs`, `EraSO.cs`, `WorldState.cs`, `EffectSO.cs`, `TimelineTriggerSO.cs`, `TimelineService.cs`, `NationSO.cs`, `NationEraProfileSO.cs`, `GameConfigSO.cs`, `RunManager.cs`, `HomeManager.cs`, `TitleSceneController.cs`, `EndingService.cs`, `EndingSO.cs`, `DebugPanelController.cs`, `DevToolsState.cs`, `ContentLibraryValidator.cs`, `docs/FEATURES.md`, `GameConfig_Default.asset`, `Trigger_*.asset`, `Ending_*.asset`.
- **LF:** `WorldContentGenerator.cs`, `world_source.json`, `FactTable.cs`, `Forgery.cs`, `FactTableTests.cs`, `ForgeryTests.cs`, `ReferenceBookSO.cs`, `ContentLibrary_Main.asset`; piece 3's new `Gates.cs`, `EffectOps.cs`, `GatesTests.cs` and `EffectOpsTests.cs`; piece 4's new `LookData.cs`, `Looks.cs`, `LookKeys.cs`, `LooksTests.cs` and `LookKeysTests.cs`.
...
- **Looks** (piece 4's LF files; R18 delivers its R27):
...
- **Checks** (a new `CheckHistory(src, authored, errors)` called from `Generate` with `CheckReferences`, before anything is written):
  ...
  - **premades** (R22): no premade's `place` or `truePlace` is in the Future era ("the Future place is in the world only while its nation leads");
  - **wardrobe items** (R18): a non-blank `artNation` passes `LookKeys.IsToken`.
...
- **Builders:**
  ...
  - `MakePlace` (221-247): no baselines for a Future place (R3), with a comment; birth years from `year` as today; piece 4's wardrobe writer copies `artNation` onto each `LookItem`;
...
- **`CheckFuture`:** at most one era `isFuture`; if one exists, every nation has a place in it; every nation has a `leaderEffect` with channel UI and a `Cue` op for which `CultureCue.TryParse` gives that nation's id (warning); a Future place with baselines (warning: it would join the tiers); a library premade (`LegendarySO`) whose claimed era (`trueEra`) or `truePlace` era is the Future (error, R22); a wardrobe item whose `artNation` fails `LookKeys.IsToken` (error, R18).
...
**`days[]`** (new, each era weight 1, `countries: []`, `tells: 1`, `channels` = day 3's, and piece 4's premade fields: `premades` = day 3's pool of seven, `forced: []`, `premadeChance` 0.05, R22):
...
| Future art sharing | piece 4's `LookItem.artNation` | `neutral` on Future hair and facial hair | `world_source.json` wardrobes |
...
**Piece 4 (characters)**, matching its R1, R27 and §2.20:
...
| `Assets/Scripts/Domain/LookData.cs`, `Looks.cs`, `LookKeys.cs` (piece 4, LF) | Domain | `LookItem.artNation`/`ArtNation`; `Compose` and `Required` use it; `CanLeak` row 6; `Looks.MaxCultureLength` removed (§2.5, R18, R20) |
...
| `Assets/Editor/WorldContentGenerator.cs` | Editor | §2.12 (including piece 3's dialog-effect and line-length checks, piece 4's `ItemData.artNation` and Culture-length constant) |
| `Assets/Editor/ContentLibraryValidator.cs` | Editor | §2.13 (and piece 4's Culture-length constant) |
...
| `Assets/Tests/EditMode/LooksTests.cs`, `LookKeysTests.cs` (piece 4, LF) | tests | §5 |
...
Line numbers are from `efe385d`; pieces 3 and 4 edit some of these lines first, and piece 5's text replaces theirs where both touch a line.
...
- **:95:** add "the eight Future places (their wardrobes share a neutral Future hair), the history rules and lines, one leader effect per country, days 4–6; also owns `Assets/Data/World/History`; generated triggers and effects are merged after the hand-authored ones; rejects fact values over 28 characters, non-ASCII history text, history values that would overflow a transcript line, a dialog effect holding SetFact and a premade claiming or coming from the Future".
- **:96:** add "the Future (one place per country, leader effects, checks per possible leader, Future days allow every country, no nation+era rule on the Future, no Future premade), history values unique (tested: `HistoryChecksTests`), SetFact only in one-shot history-rule triggers, trigger/effect/ending field checks (an epilogue needs a milestone ending)".
...
- **`LooksTests`** (piece 4's, extended): `Compose` with an `artNation` item names the part `hair_m_neutral_future_{colour}` and keeps the traveller's colour; `CanLeak` row 6: a home signature item drawing the same art as the claim's item in that slot → false; the same `artNation` in another era → true; one wig and one non-wig → true; the claim with no item in that slot → true.
- **`LookKeysTests`** (piece 4's, extended): `Required` with an `artNation` item uses that token; two places sharing it yield the same hair names; `Required`'s per-place counts are unchanged.
...
0. **Baseline, before any piece-5 code** (at the piece-4 head): dump seeds 12345 and 999, days 1–3: per slot the claim, name, registered date, role, `claimAllowed`, violator slot, liar, home, tells, answers and every paper value; and the dominance news of one scripted night.
1. **Content:**
   ...
   - a deliberately broken JSON copy (a duplicate rule value, a 29-character fact, a rule naming an unknown place, a non-ASCII rule value, a premade whose `truePlace` is a Future place, a dialog choice naming an `Effect_History_*` effect) aborts with those errors and writes nothing;
   - Validate Content Library reports no issues; its character-art report lists one neutral Future hair set (`hair_{g}_neutral_future_{colour}`), not one per country.
