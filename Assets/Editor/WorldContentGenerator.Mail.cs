...
/// world_source.json "mail" (each message's id, first and last day, flag,
...
/// reads it beside the day's generated messages. The section is read on its
/// own from the source file, so the source's shape class stays as it was.
/// </summary>
public static partial class WorldContentGenerator
{
    /// <summary>The source file's "mail" section, read on its own.</summary>
    [Serializable] private sealed class MailSourceData { public AuthoredMail[] mail; }
    ...
    private static AuthoredMail[] CheckMail(List<string> errors)
    {
        ...
        try
        {
            mail = JsonUtility.FromJson<MailSourceData>(File.ReadAllText(full))?.mail ?? Array.Empty<AuthoredMail>();
        }
        catch (Exception e)
        {
            errors.Add($"'{SourcePath}' \"mail\" cannot be read: {e.Message}");
