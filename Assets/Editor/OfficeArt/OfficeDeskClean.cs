...
public static class OfficeDeskClean
{
    ...
    static Binding[] Bindings()=>new[]{
        ...
        new Binding("ImportedOfficeDress/Desk/Clerk hotline","Phone",-.43f,.20f,1.85f,0),
        ...
        new Binding("ImportedOfficeDress/Desk/Spare forms","Paper2D",2.37f,.20f,1.20f,-10,1.063f),
    ...
    static readonly Dictionary<string,(string hex,float rough,float metal)> Palette=new(){
        ...
        {"PhoneBody",("637A69",.69f,0)},{"PhoneDial",("FFFFFF",.80f,0)},{"Paper2D",("FFFFFF",.94f,0)}
    };
    ...
    public static void Apply()=>ApplyBindings(Bindings());
    [MenuItem("Tools/Office Art/Apply Desk Anime Revision")]
    public static void ApplyRevision()=>ApplyBindings(Bindings().Where(b=>b.model=="Phone" || b.model=="Mouse" || b.model=="Paper2D").ToArray());
    static void ApplyBindings(Binding[] bindings)
    {
        ...
        var disks=GameObject.Find("HybridOffice/Booth/Finish_ComputerMedia");
        if(disks)Undo.DestroyObjectImmediate(disks);
        ...
        foreach(var entry in Palette)
        {
            ...
            if(entry.Key=="PhoneDial")mat.SetTexture("_BaseMap",AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/3DreaMax Studio/036_Retro Office Props Pack Vol-1/Art/Textures/2K/T_Landline_Phone_Basecolor.png"));
            if(entry.Key=="Paper2D")
            {
                const string texturePath=Folder+"/Textures/PaperDetail2D.png";
                var ti=(TextureImporter)AssetImporter.GetAtPath(texturePath);ti.alphaIsTransparency=true;
                ti.mipmapEnabled=true;ti.textureCompression=TextureImporterCompression.Uncompressed;ti.SaveAndReimport();
                mat.SetTexture("_BaseMap",AssetDatabase.LoadAssetAtPath<Texture2D>(texturePath));
                mat.SetFloat("_AlphaClip",1);mat.EnableKeyword("_ALPHATEST_ON");mat.SetFloat("_Cutoff",.4f);
            }
