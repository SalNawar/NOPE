...
public static partial class OfficeSceneUIBuilder
{
    ...
    /// <summary>The steps' references into the app (the app, each pane's Records, the toast, the knobs), the toolbar's Steps toggle calling StepsPanel.Toggle, and the steps for the app's key (Ctrl+Shift+S).</summary>
    private static void WireStepsPanel(StepsPanel steps, AppParts parts, Button toggle, AppToast toast, DesktopConfigSO config)
    {
        ...
        SerializedArrays.Set(so, "records", parts.Records);
        Wire(so, "toast", toast);
    ...
    /// <summary>
    /// Settings' Investigation section's Steps shown / Steps hidden pair (SG1,
    /// §2.15), on the right half of the Text size row (phase 20 builds the
    /// heading and the sizes), wired into the window's controller (its
    /// checklist: WireStepsSettings). Existing objects are kept; the anchors
    /// are re-applied.
    /// </summary>
