...
/// (the reference books' rows) and citizen registry (the Citizen Records
/// app), set once a day by GameManager through the façade, and the shelf of
...
public sealed class DayReference
{
    ...
    /// <summary>Today's directives as the Directives window shows them.</summary>
    public string Directives => _directives;

    /// <summary>Today's facts (null before the first day).</summary>
    public FactTable Facts => _facts;

    /// <summary>Today's citizen registry (null before the first day).</summary>
    ...
    /// <summary>Sets the day's citizen registry and hands it to the Records app.</summary>
    public void SetCitizenRegistry(CitizenRegistry registry)
    {
        _registry = registry;
        if (_records != null)
            _records.SetRegistry(registry);
    }
