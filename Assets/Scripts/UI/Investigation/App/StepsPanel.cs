...
/// the player sees: the Rules tab or a scanned paper shown in either pane
/// while the PC is looked at, a record looked up in either pane's Records).
/// The tick box ticks or unticks a step by hand for the rest of the case; the
/// label jumps where the step is done (CaseSteps.Target: a tab, a record
/// looked up, a book), through the panes' one navigation (phase 18's
/// InvestigationApp.Open: the active pane, its history), or, for work at the
...
public sealed class StepsPanel : MonoBehaviour
{
    ...
    [Header("The app")]
    /// <summary>The app: what the player sees (its panes' tabs and scanned paper), and where a jump goes.</summary>
    [SerializeField] private InvestigationApp app;

    /// <summary>Each pane's Records lookup: a lookup ticks "a record looked up".</summary>
    [SerializeField] private CitizenRecordsWindowController[] records = new CitizenRecordsWindowController[0];
    ...
    private void OnDestroy()
    {
        if (!_wired)
            return;
        foreach (CitizenRecordsWindowController lookup in records)
            if (lookup != null)
                lookup.Searched -= HandleSearched;
    }
    ...
    private void LateUpdate()
    {
        ...
        int tab = looking && app.Sees(AppTab.Rules) ? (int)AppTab.Rules : -1;
        int paper = looking ? app.SeenPaper() : -1;
    ...
    private void Jump(int index)
    {
        ...
        switch (target.Kind)
        {
            ...
            case StepTargetKind.Record:
                ...
                Changed(_progress.RecordViewed());
                break;
