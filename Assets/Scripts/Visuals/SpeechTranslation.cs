...
public sealed class SpeechTranslation
{
    ...
    /// <summary>A transcript line, settled: plain when not foreign, not in the tongue (<paramref name="inTongue"/> false: the desk's) or translated; else untranslated.</summary>
    public Reveal Line(bool inTongue) =>
        !_foreign || !inTongue || _translated ? Reveal.Plain : Reveal.Untranslated(_text);

    /// <summary>The bubble's traveller line <paramref name="lineSeconds"/> after it started: plain when not foreign, flipping on the line's clock when translated, else untranslated.</summary>
    public Reveal Bubble(float lineSeconds)
    {
        if (!_foreign)
            return Reveal.Plain;
        return _translated ? Reveal.Flipping(_text, lineSeconds) : Reveal.Untranslated(_text);
    }
