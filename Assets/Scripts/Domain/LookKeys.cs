...
public static class LookKeys
{
    ...
    public static LookKey Body(TravellerGender gender, int skin) =>
        new LookKey($"body_{GenderToken(gender)}_skin{skin}", LookLayer.Body, null, null, skin, null, null, null, null);

    /// <summary>The head: "head_{g}_skin{N}_face{v}".</summary>
    public static LookKey Head(TravellerGender gender, int skin, string face) =>
        new LookKey($"head_{GenderToken(gender)}_skin{skin}_face{face}", LookLayer.Head, null, null, skin, face, null, null, null);
    ...
    public static LookKey Garment(LookLayer layer, TravellerGender gender, string nationId, string eraId, string colour)
    {
        ...
        if (colour != null)
            name += "_" + colour;
        return new LookKey(name, layer, nationId, eraId, 0, null, colour, null, null);
    }
    ...
    public static LookKey Premade(string premadeId, string expression, string claimNationId, string claimEraId) =>
        new LookKey($"premade_{premadeId}_{expression}", LookLayer.Whole, claimNationId, claimEraId, 0, null, null, premadeId, expression);
