...
public enum DesktopEscape
{
    ...
    /// <summary>A window drag is under way: it is cancelled, the window back where it started.</summary>
    CancelDrag
}
...
public readonly struct DesktopEscapeState
{
    ...
    /// <summary>A window is being dragged by its title bar.</summary>
    public readonly bool Dragging;

    /// <summary>Creates a state.</summary>
    public DesktopEscapeState(bool fieldFocused, bool startMenuOpen, bool dragging)
    {
...
/// first of: leave a focused field, close the Start menu, cancel a drag, else
/// None. The desktop's poller takes the press when the rule returns something
/// and stamps the frame, so the PC frame's own Escape (OfficeViewController)
/// skips that press. Later phases add the context menu, the shortcut card and
/// the search panel at their places in the chain. Pure.
/// </summary>
