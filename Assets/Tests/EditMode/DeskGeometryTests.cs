...
/// centred on (1, -1): x -1..3, y -2..0) and its union with a scanner's
/// drop area, the window clamp and the paper stack.
/// </summary>
public class DeskGeometryTests
{
    ...
    [Test]
    public void Union_HoldsBothRectangles_AndNothingMore()
    {
        var mat = new DeskRect(0f, -0.52f, 1.6f, 1f);
        var scanner = new DeskRect(1.1f, -0.3f, 0.4f, 0.3f);
        DeskRect both = DeskRect.Union(mat, scanner);

        Assert.AreEqual(-0.8f, both.CentreX - both.Width / 2f, 1e-5f);
        Assert.AreEqual(1.3f, both.CentreX + both.Width / 2f, 1e-5f);
        Assert.AreEqual(-1.02f, both.CentreY - both.Height / 2f, 1e-5f);
        Assert.AreEqual(-0.02f, both.CentreY + both.Height / 2f, 1e-5f);
        Assert.IsTrue(both.Contains(1.25f, -0.2f));
        Assert.IsFalse(both.Contains(1.35f, -0.2f));
    }

    [Test]
    public void Union_WithAContainedRectangle_IsTheOuterOne()
    {
        var outer = new DeskRect(1f, 2f, 4f, 2f);
        DeskRect both = DeskRect.Union(outer, new DeskRect(1.5f, 2f, 1f, 1f));
        Assert.AreEqual(new[] { 1f, 2f, 4f, 2f }, new[] { both.CentreX, both.CentreY, both.Width, both.Height });
    }
